(function (global, factory) {
  ...
  function enableSearchInput (enabled) {
    ...
    searchInput.placeholder = enabled ? 'Search the docs' : 'Loading index…';
  }
  ...
  function multiExecuteSearch () {
    ...
    if (modulesLoaded < totalModules) {
      ...
      loadingNotice.classList.add('search-result-loading-notice');
      loadingNotice.innerText = `Loading search index (${modulesLoaded}/${totalModules})...`;
      searchResultContainer.appendChild(loadingNotice);
    }
    let any = false;
    const frag = document.createDocumentFragment();
  ...
  async function bootstrap (lunr, manifest, siteRootPath) {
    ...
    try {
      ...
      totalModules = manifestObj.modules.length;
      if (!first) return
      ...
      const startBackgroundOnce = (() => {
        ...
        const loadNext = async (i) => {
          ...
          try {
            await loadModuleEntry(lunr, rest[i]);
            if (searchInput.value && !isClosed()) multiExecuteSearch();
          } catch (e) {}
