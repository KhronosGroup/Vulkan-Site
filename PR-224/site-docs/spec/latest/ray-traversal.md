...
- [Determination](#_determination_2)
- [Culling](#_culling_2)
- [Ray Query](#_ray_query)
...
![ray tracing execution detail](../_images/ray_tracing_execution_detail.svg)

Figure 1. Ray tracing traversal
...
Figure 2. Ray intersection candidate
...
These values are included in the triangle intersection candidate.
...
determined as opaque or non-opaque as described in [Ray Opacity Micromap](#ray-opacity-micromap).
If the result of the opacity micromap operation is *ignored*, the
...
[VkAccelerationStructureInstanceKHR](accelstructures.html#VkAccelerationStructureInstanceKHR)::`flags` will override all
...
Including `OpaqueKHR` in the `Ray Flags` parameter when
tracing a ray
or
initializing a ray query object
overrides all non-opaque geometries to behave as if they are opaque,
ignoring any build instance flag overrides.
Similarly, `NoOpaqueKHR` overrides all opaque geometries to behave as if
they are non-opaque.

If `CullOpaqueKHR` is included in the `Ray Flags` parameter when
tracing a ray
or
initializing a ray query object,
...
Similarly, `CullNoOpaqueKHR` drops intersections with geometry considered
to be non-opaque.
...
`OpRayQueryProceedKHR` when using it to trace rays.

Intersection candidates with AABB geometry cause proceed to return
`true`, incomplete traversal, in order for the shader to confirm the hit.
...
`true` have been determined to be non-opaque.
...
micromap and [VK_GEOMETRY_INSTANCE_DISABLE_OPACITY_MICROMAPS_BIT_KHR](accelstructures.html#VkGeometryInstanceFlagBitsNV) is
...
Using an acceleration structure that contains opacity micromap data requires
the shader or pipeline to declare that it does so.
For traversals performed by [pipeline trace ray](../appendices/glossary.html#glossary-pipeline-trace-ray) instructions the pipeline **must** be created with
[VK_PIPELINE_CREATE_RAY_TRACING_OPACITY_MICROMAP_BIT_KHR](pipelines.html#VkPipelineCreateFlagBits).
For traversals performed by `OpRayQueryProceedKHR`
where [VK_EXT_opacity_micromap](../appendices/extensions.html#VK_EXT_opacity_micromap) is not enabled,
the shader **must** declare the `OpacityMicromapIdKHR` execution mode with
its specialization-constant operand equal to [VK_TRUE](fundamentals.html#VK_TRUE).
If [VK_EXT_opacity_micromap](../appendices/extensions.html#VK_EXT_opacity_micromap) is enabled, `OpRayQueryProceedKHR`
traversal always uses opacity micromap data without the
`OpacityMicromapIdKHR` execution mode being declared.
This applies whenever [VK_EXT_opacity_micromap](../appendices/extensions.html#VK_EXT_opacity_micromap) is enabled, including
when [VK_KHR_opacity_micromap](../appendices/extensions.html#VK_KHR_opacity_micromap) is enabled as well.
A shader executed in a ray tracing pipeline that itself performs ray query
operations is subject to both requirements independently, each applying only
to the traversal it governs.

|  | These declarations are requirements, not a means of disabling opacity
| --- | --- |
micromaps.
Traversing an acceleration structure that contains opacity micromap data
without the applicable declaration has **undefined** results, and is not
equivalent to ignoring the opacity micromap data.
To traverse such an acceleration structure without applying its opacity
micromaps, set [VK_GEOMETRY_INSTANCE_DISABLE_OPACITY_MICROMAPS_BIT_KHR](accelstructures.html#VkGeometryInstanceFlagBitsNV)
...
Figure 3. Example ordering for micromap data
...
[VK_OPACITY_MICROMAP_FORMAT_2_STATE_KHR](VK_KHR_opacity_micromap/micromaps.html#VkOpacityMicromapFormatEXT), each element of the micromap
is represented by a single bit at the index derived above.

If the associated opacity micromap has format
[VK_OPACITY_MICROMAP_FORMAT_4_STATE_KHR](VK_KHR_opacity_micromap/micromaps.html#VkOpacityMicromapFormatEXT), each element is represented by
a two bit value at the index derived above.

If the micromap was built with
[VK_BUILD_ACCELERATION_STRUCTURE_MICROMAP_LOSSY_BIT_KHR](resources.html#VkBuildAccelerationStructureFlagBitsNV) and has a
format of [VK_OPACITY_MICROMAP_FORMAT_4_STATE_KHR](VK_KHR_opacity_micromap/micromaps.html#VkOpacityMicromapFormatEXT), the implementation
**may** substitute any element’s state value with
[VK_OPACITY_MICROMAP_SPECIAL_INDEX_FULLY_UNKNOWN_TRANSPARENT_KHR](accelstructures.html#VkOpacityMicromapSpecialIndexEXT) or
[VK_OPACITY_MICROMAP_SPECIAL_INDEX_FULLY_UNKNOWN_OPAQUE_KHR](accelstructures.html#VkOpacityMicromapSpecialIndexEXT).
The application **should** accept the hit if the element was built with
[VK_OPACITY_MICROMAP_SPECIAL_INDEX_FULLY_OPAQUE_KHR](accelstructures.html#VkOpacityMicromapSpecialIndexEXT) and ignore the hit
if the element was built with
[VK_OPACITY_MICROMAP_SPECIAL_INDEX_FULLY_TRANSPARENT_KHR](accelstructures.html#VkOpacityMicromapSpecialIndexEXT).
...
| --- | --- | --- | --- |
| 0 | 0 | [VK_OPACITY_MICROMAP_SPECIAL_INDEX_FULLY_TRANSPARENT_KHR](accelstructures.html#VkOpacityMicromapSpecialIndexEXT) | Ignored |
| 1 | 1 | [VK_OPACITY_MICROMAP_SPECIAL_INDEX_FULLY_OPAQUE_KHR](accelstructures.html#VkOpacityMicromapSpecialIndexEXT) | Opaque |
| 2 |  | [VK_OPACITY_MICROMAP_SPECIAL_INDEX_FULLY_UNKNOWN_TRANSPARENT_KHR](accelstructures.html#VkOpacityMicromapSpecialIndexEXT) | Non-opaque |
| 3 |  | [VK_OPACITY_MICROMAP_SPECIAL_INDEX_FULLY_UNKNOWN_OPAQUE_KHR](accelstructures.html#VkOpacityMicromapSpecialIndexEXT) | Non-opaque |

Unless if the
[VK_GEOMETRY_INSTANCE_FORCE_OPACITY_MICROMAP_2_STATE_BIT_KHR](accelstructures.html#VkGeometryInstanceFlagBitsNV) flag is
included in the instance flags of the candidate intersection or
`ForceOpacityMicromap2StateEXT` is included in the ray flags on the ray,
...
| 0 | 0 | [VK_OPACITY_MICROMAP_SPECIAL_INDEX_FULLY_TRANSPARENT_KHR](accelstructures.html#VkOpacityMicromapSpecialIndexEXT) | Ignored |
| 1 | 1 | [VK_OPACITY_MICROMAP_SPECIAL_INDEX_FULLY_OPAQUE_KHR](accelstructures.html#VkOpacityMicromapSpecialIndexEXT) | Opaque |
| 2 |  | [VK_OPACITY_MICROMAP_SPECIAL_INDEX_FULLY_UNKNOWN_TRANSPARENT_KHR](accelstructures.html#VkOpacityMicromapSpecialIndexEXT) | Ignored |
| 3 |  | [VK_OPACITY_MICROMAP_SPECIAL_INDEX_FULLY_UNKNOWN_OPAQUE_KHR](accelstructures.html#VkOpacityMicromapSpecialIndexEXT) | Opaque |

|  | The Force2state override never evaluates to *non-opaque* result, even when
| --- | --- |
...
distinct states.
A candidate intersection is available only while `OpRayQueryProceedKHR`
has returned `true`, and identifies the intersection currently requiring
shader evaluation.
Calling `OpRayQueryGenerateIntersectionKHR` or
`OpRayQueryConfirmIntersectionKHR` confirms a hit for
...
The generated intersection candidate includes the values stored in variables
with the `HitAttributeKHR` storage class at the time
`OpReportIntersectionKHR` is executed.
...
`OpRayQueryGenerateIntersectionKHR` **can** be called to report a generated
...
intersection type is `RayQueryCommittedIntersectionGeneratedKHR`.
...
confirm the candidate as a hit for [Ray Closest Hit Determination](#ray-closest-hit-determination).
If that hit becomes the current closest hit, the ray query’s committed
intersection type is `RayQueryCommittedIntersectionTriangleKHR`.
Further ray query processing **can** be continued by executing
`OpRayQueryProceedKHR` with the same ray query, or intersection **can** be
terminated with `OpRayQueryTerminateKHR`.
...
`OpRayQueryGetIntersection*KHR` instructions with `Intersection` set to
`RayQueryCommittedIntersectionKHR` are updated to describe that hit.
...
If a closest hit was identified by [Ray Closest Hit Determination](#ray-closest-hit-determination), the ray query’s committed intersection is the
