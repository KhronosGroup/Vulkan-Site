...
* 
`flags` is a bitmask of [VkPrivateDataSlotCreateFlagBits](#VkPrivateDataSlotCreateFlagBits)
...
[](#VUID-VkPrivateDataSlotCreateInfo-flags-12501) VUID-VkPrivateDataSlotCreateInfo-flags-12501
  ...
  If `flags` contain
  [VK_PRIVATE_DATA_SLOT_CREATE_BASE_OBJECT_HANDLE_BIT_NV](#VkPrivateDataSlotCreateFlagBitsEXT), the
[`privateDataBaseHandle`](features.html#features-privateDataBaseHandle) feature
**must** be enabled
...
[](#VUID-VkPrivateDataSlotCreateInfo-flags-parameter) VUID-VkPrivateDataSlotCreateInfo-flags-parameter
 ...
 If `flags` is not `0`, `flags` **must** be a valid combination of [VkPrivateDataSlotCreateFlagBits](#VkPrivateDataSlotCreateFlagBits) values
...
`VkPrivateDataSlotCreateFlags` is a bitmask type for setting a mask of
zero or more [VkPrivateDataSlotCreateFlagBits](#VkPrivateDataSlotCreateFlagBits).

Bits which **can** be set in [VkPrivateDataSlotCreateInfo](#VkPrivateDataSlotCreateInfo)::`flags`,
...
* 
[VK_PRIVATE_DATA_SLOT_CREATE_BASE_OBJECT_HANDLE_BIT_NV](#VkPrivateDataSlotCreateFlagBitsEXT) specifies
...
* 
[](#VUID-vkSetPrivateData-privateDataSlot-12502) VUID-vkSetPrivateData-privateDataSlot-12502

`privateDataSlot` **must** not have been created with
[VK_PRIVATE_DATA_SLOT_CREATE_BASE_OBJECT_HANDLE_BIT_NV](#VkPrivateDataSlotCreateFlagBitsEXT)
...
returned.
If `privateDataSlot` was not created with
[VK_PRIVATE_DATA_SLOT_CREATE_BASE_OBJECT_HANDLE_BIT_NV](#VkPrivateDataSlotCreateFlagBitsEXT), `0` will be
written in the absence of a previous call to `vkSetPrivateData`
using the object specified by `objectHandle`.
If `privateDataSlot` was created with
[VK_PRIVATE_DATA_SLOT_CREATE_BASE_OBJECT_HANDLE_BIT_NV](#VkPrivateDataSlotCreateFlagBitsEXT), the
implementation’s base handle for the object is written to `pData`.
The base handle is the value of `objectHandle` as received by the
