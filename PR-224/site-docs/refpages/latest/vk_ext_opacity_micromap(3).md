...
* 
The two extensions differ in how opacity micromap data is consumed by
ray query traversal.
With [VK_EXT_opacity_micromap](#) enabled, opacity micromap data
present in the acceleration structure is always consumed.
With only [VK_KHR_opacity_micromap](VK_KHR_opacity_micromap.html) enabled and its `micromap`
feature enabled, opacity micromap data is consumed only when the shader
declares the `OpacityMicromapIdKHR` execution mode with its
specialization-constant operand equal to [VK_TRUE](VK_TRUE.html).
If both extensions are enabled, the [VK_EXT_opacity_micromap](#)
behavior applies and the execution mode is not required.
Traversal initiated by [pipeline trace    ray](../../../../spec/latest/appendices/glossary.html#glossary-pipeline-trace-ray) instructions is unaffected by this difference; it requires
[VK_PIPELINE_CREATE_RAY_TRACING_OPACITY_MICROMAP_BIT_KHR](VkPipelineCreateFlagBits.html) under
either extension.
