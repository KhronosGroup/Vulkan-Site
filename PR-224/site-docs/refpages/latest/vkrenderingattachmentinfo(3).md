...
If `resolveMode` is not [VK_RESOLVE_MODE_NONE](VkResolveModeFlagBits.html),
[multisampled rendering to single-sampled attachments](../../../../spec/latest/chapters/renderpass.html#multisampled-render-to-single-sampled) is enabled, and `imageView` has a sample
count of [VK_SAMPLE_COUNT_1_BIT](VkSampleCountFlagBits.html), `resolveImageView` **must** be
[VK_NULL_HANDLE](VK_NULL_HANDLE.html).
Otherwise,
`resolveImageView` **must** not be [VK_NULL_HANDLE](VK_NULL_HANDLE.html).

If `resolveMode` is not [VK_RESOLVE_MODE_NONE](VkResolveModeFlagBits.html), a
...
* 
`storeOp` is [VK_ATTACHMENT_STORE_OP_STORE](VkAttachmentStoreOp.html)
...
* 
`resolveImageView` is not [VK_NULL_HANDLE](VK_NULL_HANDLE.html)

* 
`resolveMode` is not [VK_RESOLVE_MODE_NONE](VkResolveModeFlagBits.html)

* 
`resolveMode` is not [VK_RESOLVE_MODE_CUSTOM_BIT_EXT](VkResolveModeFlagBits.html)
...
[](#VUID-VkRenderingAttachmentInfo-imageView-06136) VUID-VkRenderingAttachmentInfo-imageView-06136
    ...
    If `resolveImageView` is not [VK_NULL_HANDLE](VK_NULL_HANDLE.html),
    `resolveImageLayout` **must** not be [VK_IMAGE_LAYOUT_UNDEFINED](VkImageLayout.html),
...
created with a [VkImageViewCreateInfo](VkImageViewCreateInfo.html)::`viewType` not equal to
...
If `resolveImageView` is not [VK_NULL_HANDLE](VK_NULL_HANDLE.html), it **must** have
been created with a [VkImageViewCreateInfo](VkImageViewCreateInfo.html)::`viewType` not
equal to [VK_IMAGE_VIEW_TYPE_3D](VkImageViewType.html)

[](#VUID-VkRenderingAttachmentInfo-imageView-12486) VUID-VkRenderingAttachmentInfo-imageView-12486

If `imageView` is not [VK_NULL_HANDLE](VK_NULL_HANDLE.html) it **must** only specify a
single mip level

[](#VUID-VkRenderingAttachmentInfo-resolveImageView-12487) VUID-VkRenderingAttachmentInfo-resolveImageView-12487

If `resolveImageView` is not [VK_NULL_HANDLE](VK_NULL_HANDLE.html) it **must** only
