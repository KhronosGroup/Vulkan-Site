...
the execution of any earlier [vkCmdWaitEvents](vkCmdWaitEvents.html) in
...
There **must** be an execution dependency between `vkCmdResetEvent` and
the execution of any earlier [vkCmdWaitEvents2](vkCmdWaitEvents2.html) in
[submission order](../../../../spec/latest/chapters/synchronization.html#synchronization-submission-order) that includes
`event` in its `pEvents` parameter
...
[](#VUID-vkCmdResetEvent-stageMask-parameter) VUID-vkCmdResetEvent-stageMask-parameter
 ...
 If `stageMask` is not `0`, `stageMask` **must** be a valid combination of [VkPipelineStageFlagBits](VkPipelineStageFlagBits.html) values
