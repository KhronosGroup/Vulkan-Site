...
If the parameters define an import operation and the external handle
type is [VK_EXTERNAL_MEMORY_HANDLE_TYPE_DMA_BUF_BIT_EXT](VkExternalMemoryHandleTypeFlagBits.html), the number
...
If [VkMemoryAllocateFlagsInfo](VkMemoryAllocateFlagsInfo.html)::`flags` does not include
