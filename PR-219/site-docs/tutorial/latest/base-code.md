...
The `#if defined(*INTELLISENSE*) || !defined(USE_CPP20_MODULES)` block picks
...
traditional `` header. If you enable C++20 modules
...
to the header whenever `*INTELLISENSE*` is defined (which IntelliSense
...
|  | On Wayland, the compositor won’t actually map (show) a window until its
| --- | --- |
