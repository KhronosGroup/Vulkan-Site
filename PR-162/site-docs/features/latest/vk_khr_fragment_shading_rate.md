...
This state can be set per-draw as part of the pipeline by chaining the following structure to [VkGraphicsPipelineCreateInfo](../../../../refpages/latest/refpages/source/VkGraphicsPipelineCreateInfo.html):
...
In each case, the [VkExtent2D](../../../../refpages/latest/refpages/source/VkExtent2D.html) sets the base fragment size in the x and y dimensions for all fragments generated by draw calls using this state.
...
This image can be set per-subpass by chaining the following structure to [VkSubpassDescription2](../../../../refpages/latest/refpages/source/VkSubpassDescription2.html):
...
* 
`fragmentShadingRateWithSampleMask` determines if the `pSampleMask` member of [VkPipelineMultisampleStateCreateInfo](../../../../refpages/latest/refpages/source/VkPipelineMultisampleStateCreateInfo.html) can have any valid bits equal to 0 when using with fragment shading rate.
