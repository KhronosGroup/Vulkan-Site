...
Since then however, `VK_QUEUE_FAMILY_EXTERNAL` and `VK_QUEUE_FAMILY_FOREIGN_EXT` in [queue family ownership transfers](../../../../spec/latest/chapters/synchronization.html#synchronization-queue-transfers) provided a better mechanism to interact with such external entities.
