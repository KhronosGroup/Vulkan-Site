...
- **Component**: tutorial
- **Version**: latest
- **URL**: /tutorial/latest/Building_a_Simple_Engine/Loading_Models/07_scene_rendering.html

## Table of Contents

- [Rendering the Scene](#_rendering_the_scene)
- [Rendering_the_Scene](#_rendering_the_scene)
- [Introduction to Scene Rendering](#_introduction_to_scene_rendering)
- [Introduction_to_Scene_Rendering](#_introduction_to_scene_rendering)
- [Scene Graph Traversal](#_scene_graph_traversal)
- [Scene_Graph_Traversal](#_scene_graph_traversal)
- [Node traversal and transform calculation](#_node_traversal_and_transform_calculation)
- [Node_traversal_and_transform_calculation](#_node_traversal_and_transform_calculation)
- [Mesh validation and rendering preparation](#_mesh_validation_and_rendering_preparation)
- [Mesh_validation_and_rendering_preparation](#_mesh_validation_and_rendering_preparation)
- [Material property configuration](#_material_property_configuration)
- [Material_property_configuration](#_material_property_configuration)
- [GPU resource binding and draw command execution](#_gpu_resource_binding_and_draw_command_execution)
- [GPU_resource_binding_and_draw_command_execution](#_gpu_resource_binding_and_draw_command_execution)
- [Hierarchical recursion](#_hierarchical_recursion)
- [Understanding the Rendering Process](#_understanding_the_rendering_process)
- [Understanding_the_Rendering_Process](#_understanding_the_rendering_process)
- [Transformation Calculation](#_transformation_calculation)
- [Material Setup](#_material_setup)
- [Mesh Rendering](#_mesh_rendering)
- [Recursive Traversal](#_recursive_traversal)
- [Integrating Scene Rendering in the Main Loop](#_integrating_scene_rendering_in_the_main_loop)
- [Integrating_Scene_Rendering_in_the_Main_Loop](#_integrating_scene_rendering_in_the_main_loop)
- [Performance Considerations](#_performance_considerations)
- [Frustum Culling](#_frustum_culling)
- [Level of Detail (LOD)](#_level_of_detail_lod)
- [Level_of_Detail_(LOD)](#_level_of_detail_lod)
- [Occlusion Culling](#_occlusion_culling)
- [Instanced Rendering](#_instanced_rendering)
- [Advanced Scene Rendering Techniques](#_advanced_scene_rendering_techniques)
- [Advanced_Scene_Rendering_Techniques](#_advanced_scene_rendering_techniques)
- [Hierarchical Culling](#_hierarchical_culling)
- [Deferred Rendering](#_deferred_rendering)
- [Clustered Rendering](#_clustered_rendering)
...
void renderNode(const vk::raii::CommandBuffer& commandBuffer, const std::vector& nodes, const glm::mat4& parentMatrix) {
...
This material setup step translates high-level material descriptions into GPU-ready push constants that control the appearance and lighting properties of the rendered geometry.
            ...
            if (node->mesh.materialIndex >= 0 && node->mesh.materialIndex (model.materials.size())) {
...
The final rendering phase binds GPU resources and issues the actual draw command that transforms the scene graph node into rendered pixels on the screen.
            ...
            commandBuffer.drawIndexed(static_cast(node->mesh.indices.size()), 1, 0, 0, 0);
        }
...
Each node’s transformation is correctly combined with its parent’s transformation

Child nodes are rendered with the correct global transformation

The scene graph hierarchy is preserved during rendering
...
|  | We covered PBR material theory and shader details earlier in [PBR Rendering](05_pbr_rendering.html), so we won’t restate that here. This section focuses on the wiring: how material properties are packed into push constants and consumed by the draw call in this chapter’s context. |
| --- | --- |
...
Sets up push constants with the material properties

Passes these properties to the fragment shader using push constants
...
Binds the vertex buffer containing the mesh’s vertices

Binds the index buffer containing the mesh’s indices

Issues a draw command to render the mesh
...
Sets up the Vulkan rendering state (command buffer, image transitions, rendering attachments)

Binds the graphics pipeline and descriptor sets

Updates the view and projection matrices in the uniform buffer

Calls `renderScene` to render the entire scene

Finalizes the rendering and presents the result
...
bool isNodeVisible(const Node* node, const glm::mat4& viewProjection) {
    ...
    for (int i = 0; i & nodes, const glm::mat4& parentMatrix, const glm::mat4& viewProjection) {
    for (const auto node : nodes) {
        // Calculate global matrix for this node
        glm::mat4 nodeMatrix = parentMatrix * node->getLocalMatrix();
...
void renderNodeWithLOD(const vk::raii::CommandBuffer& commandBuffer, const std::vector& nodes, const glm::mat4& parentMatrix, const glm::vec3& cameraPosition) {
    for (const auto node : nodes) {
        // Calculate global matrix for this node
        glm::mat4 nodeMatrix = parentMatrix * node->getLocalMatrix();
...
void renderNodeWithOcclusion(const vk::raii::CommandBuffer& commandBuffer, const std::vector& nodes, const glm::mat4& parentMatrix) {
    ...
    std::sort(sortedNodes.begin(), sortedNodes.end(), [](const auto& a, const auto& b) {
        return a.second getLocalMatrix();
...
void renderInstanced(const vk::raii::CommandBuffer& commandBuffer, const std::vector& nodes, const std::vector& instanceMatrices) {
    for (const auto node : nodes) {
        ...
        if (!node->mesh.vertices.empty() && !node->mesh.indices.empty() &&
            ...
            commandBuffer.drawIndexedInstanced(
                ...
                static_cast(instanceMatrices.size()),
                0, 0, 0
            );
        }
...
void renderNodeHierarchical(const vk::raii::CommandBuffer& commandBuffer, const std::vector& nodes, const glm::mat4& parentMatrix, const glm::mat4& viewProjection) {
    for (const auto node : nodes) {
        // Calculate global matrix for this node
        glm::mat4 nodeMatrix = parentMatrix * node->getLocalMatrix();
...
[Previous: Rendering Multiple Objects](06_multiple_objects.html) | [Next: Updating Animations](08_animations.html)
