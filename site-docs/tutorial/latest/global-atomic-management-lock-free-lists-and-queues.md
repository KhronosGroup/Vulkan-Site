...
- **Component**: tutorial
- **Version**: latest
- **URL**: /tutorial/latest/Advanced_Vulkan_Compute/06_Advanced_Data_Structures/03_global_atomic_management.html

## Table of Contents

- [Why 64-bit Atomics?](#_why_64_bit_atomics)
- [Why_64-bit_Atomics?](#_why_64_bit_atomics)
- [What Does "Lock-Free" Actually Mean?](#_what_does_lock_free_actually_mean)
- [What_Does_"Lock-Free"_Actually_Mean?](#_what_does_lock_free_actually_mean)
- [Building Lock-Free Linked Lists](#_building_lock_free_linked_lists)
- [Building_Lock-Free_Linked_Lists](#_building_lock_free_linked_lists)
- [The Anatomy of a GPU Linked List](#_the_anatomy_of_a_gpu_linked_list)
- [The_Anatomy_of_a_GPU_Linked_List](#_the_anatomy_of_a_gpu_linked_list)
- [Beyond Exchange: Compare-and-Swap (CAS)](#_beyond_exchange_compare_and_swap_cas)
- [Beyond_Exchange:_Compare-and-Swap_(CAS)](#_beyond_exchange_compare_and_swap_cas)
- [GLSL: atomicAdd and 64-bit Atomics](#_glsl_atomicadd_and_64_bit_atomics)
- [GLSL:_atomicAdd_and_64-bit_Atomics](#_glsl_atomicadd_and_64_bit_atomics)
- [Building Work Queues](#_building_work_queues)
- [Building_Work_Queues](#_building_work_queues)
- [The Anatomy of a Work Queue](#_the_anatomy_of_a_work_queue)
- [The_Anatomy_of_a_Work_Queue](#_the_anatomy_of_a_work_queue)
- [Optimizing Atomics with Subgroups](#_optimizing_atomics_with_subgroups)
- [Optimizing_Atomics_with_Subgroups](#_optimizing_atomics_with_subgroups)
...
**The Head Buffer**: A 2D texture or buffer (matching the screen resolution) that stores the index of the first node for each pixel. It is initialized to a "null" value (e.g., `0xFFFFFFFF`).
...
RWStructuredBuffer headBuffer; // size: width * height
RWStructuredBuffer nodePool;   // size: Max total fragments
RWStructuredBuffer counter;    // size: 1

When a fragment is processed:

The thread atomically increments the `counter` to get a unique `newNodeIdx`.

The thread uses `InterlockedExchange` on the `headBuffer` at its pixel location. It writes `newNodeIdx` and receives the `oldHead`.

The thread writes its data and the `oldHead` (as `nextIdx`) into `nodePool[newNodeIdx]`.
...
struct WorkQueue {
    RWStructuredBuffer data;    // Storage for pending tasks
    RWStructuredBuffer counter; // Number of tasks currently in the queue
};
...
[Previous: GPU-Resident Trees](02_gpu_resident_trees.html) | [Next: Device-Addressable Buffers](04_device_addressable_buffers.html)
