...
- **Component**: tutorial
- **Version**: latest
- **URL**: /tutorial/latest/Building_a_Simple_Engine/Loading_Models/06_multiple_objects.html

## Table of Contents

- [Managing Multiple Objects in a 3D Scene](#_managing_multiple_objects_in_a_3d_scene)
- [Managing_Multiple_Objects_in_a_3D_Scene](#_managing_multiple_objects_in_a_3d_scene)
- [Introduction to Multi-Object Rendering](#_introduction_to_multi_object_rendering)
- [Introduction_to_Multi-Object_Rendering](#_introduction_to_multi_object_rendering)
- [Approaches to Managing Multiple Objects](#_approaches_to_managing_multiple_objects)
- [Approaches_to_Managing_Multiple_Objects](#_approaches_to_managing_multiple_objects)
- [Object Instances vs. Multiple Models](#_object_instances_vs_multiple_models)
- [Object_Instances_vs._Multiple_Models](#_object_instances_vs_multiple_models)
- [Scene Organization Strategies](#_scene_organization_strategies)
- [Scene_Organization_Strategies](#_scene_organization_strategies)
- [Performance Considerations](#_performance_considerations)
- [Draw Call Optimization](#_draw_call_optimization)
- [Draw_Call_Optimization](#_draw_call_optimization)
- [Culling Techniques](#_culling_techniques)
- [Memory Management](#_memory_management)
- [Implementing Object Instances](#_implementing_object_instances)
- [Implementing_Object_Instances](#_implementing_object_instances)
- [Rendering Multiple Objects](#_rendering_multiple_objects)
- [Rendering_Multiple_Objects](#_rendering_multiple_objects)
- [Advanced Techniques: Hardware Instancing](#_advanced_techniques_hardware_instancing)
- [Advanced_Techniques:_Hardware_Instancing](#_advanced_techniques_hardware_instancing)
- [Vertex Shader Modifications for Instancing](#_vertex_shader_modifications_for_instancing)
- [Vertex_Shader_Modifications_for_Instancing](#_vertex_shader_modifications_for_instancing)
- [Beyond Basic Instancing: Material Variations](#_beyond_basic_instancing_material_variations)
- [Beyond_Basic_Instancing:_Material_Variations](#_beyond_basic_instancing_material_variations)
...
void drawFrame() {
    ...
    for (size_t i = 0; i & nodes, const glm::mat4& parentMatrix) {
...
std::vector instanceBuffers;
std::vector instanceBufferMemories;
std::vector instanceBuffersMapped;
...
void setupInstanceBuffers() {
    ...
    for (auto node : model.linearNodes) {
        ...
        node->instanceBufferIndex = static_cast(instanceBuffers.size() - 1);
    }
...
void updateInstanceBuffers() {
    ...
    for (auto node : model.linearNodes) {
        if (node->instanceBufferIndex  instanceData(objectInstances.size());
        for (size_t i = 0; i getLocalMatrix();
        }
...
std::array instanceAttributeDescriptions{};
for (uint32_t i = 0; i  bindingDescriptions = {
...
vertexInputInfo.vertexBindingDescriptionCount = static_cast(bindingDescriptions.size());
...
void renderNodeInstanced(const vk::raii::CommandBuffer& commandBuffer, const std::vector& nodes) {
    for (const auto node : nodes) {
        ...
        if (!node->mesh.vertices.empty() && !node->mesh.indices.empty() &&
            ...
            commandBuffer.drawIndexed(
                ...
                static_cast(objectInstances.size()),  // Instance count
                0, 0, 0
            );
        }
...
void createMaterialVariations() {
    ...
    for (auto node : model.linearNodes) {
        ...
        if (!node->mesh.vertices.empty()) {
            ...
            node->mesh.materialIndex = static_cast(materialIndex);
        }
    }
}
