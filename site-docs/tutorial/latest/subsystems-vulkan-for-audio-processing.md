...
- **Component**: tutorial
- **Version**: latest
- **URL**: /tutorial/latest/Building_a_Simple_Engine/Subsystems/03_vulkan_audio.html

## Table of Contents

- [Enhancing Audio with Vulkan](#_enhancing_audio_with_vulkan)
- [Enhancing_Audio_with_Vulkan](#_enhancing_audio_with_vulkan)
- [Understanding HRTF](#_understanding_hrtf)
- [Why Use Vulkan for Audio Processing?](#audio-vulkan-why)
- [Why_Use_Vulkan_for_Audio_Processing?](#audio-vulkan-why)
- [Implementing HRTF Processing with Vulkan](#_implementing_hrtf_processing_with_vulkan)
- [Implementing_HRTF_Processing_with_Vulkan](#_implementing_hrtf_processing_with_vulkan)
- [HRTF Compute Shader](#_hrtf_compute_shader)
- [HRTF_Compute_Shader](#_hrtf_compute_shader)
- [Performance Considerations](#_performance_considerations)
- [Integration with the Audio System](#_integration_with_the_audio_system)
- [Integration_with_the_Audio_System](#_integration_with_the_audio_system)
- [Advantages of Vulkan-Based HRTF](#_advantages_of_vulkan_based_hrtf)
- [Advantages_of_Vulkan-Based_HRTF](#_advantages_of_vulkan_based_hrtf)
...
struct HRTFData {
    std::array leftEarImpulseResponse;
    std::array rightEarImpulseResponse;
};
...
public:
    ...
    void SetHRTFDatabase(std::shared_ptr database) { m_HRTFDatabase = database; }
...
void AudioSystem::InitializeVulkanResources() {
    ...
    vk::ShaderModuleCreateInfo shaderModuleCreateInfo({}, shaderCode.size() * sizeof(uint32_t),
                                                     reinterpret_cast(shaderCode.data()));
...
void AudioSystem::Update(float deltaTime) {
    ...
    for (auto& source : m_Sources) {
        if (source->IsPlaying()) {
            ...
            ProcessAudioWithVulkan(const_cast(audioData), processedAudio, framesToProcess);
...
void AudioSystem::Initialize() {
    ...
    m_HRTFDatabase = std::make_shared("data/hrtf/default.hrtf");
    m_HRTFEnabled = true;
}
...
See the core benefits listed in [Why Use Vulkan for Audio Processing?](#audio-vulkan-why) for a summary of why compute shaders are a good fit. In the context of HRTF specifically, two practical advantages are worth highlighting:
