...
- **Component**: tutorial
- **Version**: latest
- **URL**: /tutorial/latest/Synchronization/Dynamic_Rendering_Sync/02_subpass_replacement.html

## Table of Contents

- [The End of the Subpass Dependency](#_the_end_of_the_subpass_dependency)
- [The_End_of_the_Subpass_Dependency](#_the_end_of_the_subpass_dependency)
- [A Concrete Example](#_a_concrete_example)
- [A_Concrete_Example](#_a_concrete_example)
- [Why This is Better](#_why_this_is_better)
- [Why_This_is_Better](#_why_this_is_better)
- [Simple Engine: Dynamic Rendering Sync](#_simple_engine_dynamic_rendering_sync)
- [Simple_Engine:_Dynamic_Rendering_Sync](#_simple_engine_dynamic_rendering_sync)
...
**Depth Pre-Pass**: We call `commandBuffer.beginRendering` for the depth pre-pass.

**Barrier**: After `endRendering`, we record a `depthToRead2` barrier. This barrier synchronizes the `eLateFragmentTests` (the depth writes) with the `eEarlyFragmentTests` (the depth reads) of the next pass.

**Main Opaque Pass**: We then call `beginRendering` again for our main opaque color pass, which now has safe access to the pre-filled depth buffer.
...
Previous: [Introduction](01_introduction.html) | Next: [Local Read Sync](03_local_read_sync.html)
