...
- **Component**: tutorial
- **Version**: latest
- **URL**: /tutorial/latest/Advanced_Vulkan_Compute/06_Advanced_Data_Structures/02_gpu_resident_trees.html

## Table of Contents

- [Why Trees?](#_why_trees)
- [Bounding Volume Hierarchies (BVH)](#_bounding_volume_hierarchies_bvh)
- [Bounding_Volume_Hierarchies_(BVH)](#_bounding_volume_hierarchies_bvh)
- [The Anatomy of a BVH Node](#_the_anatomy_of_a_bvh_node)
- [The_Anatomy_of_a_BVH_Node](#_the_anatomy_of_a_bvh_node)
- [Building the Hierarchy: Morton Codes](#_building_the_hierarchy_morton_codes)
- [Building_the_Hierarchy:_Morton_Codes](#_building_the_hierarchy_morton_codes)
- [Octrees](#_octrees)
- [The Anatomy of an Octree Node](#_the_anatomy_of_an_octree_node)
- [The_Anatomy_of_an_Octree_Node](#_the_anatomy_of_an_octree_node)
- [Construction Strategies: Top-Down vs. Bottom-Up](#_construction_strategies_top_down_vs_bottom_up)
- [Construction_Strategies:_Top-Down_vs._Bottom-Up](#_construction_strategies_top_down_vs_bottom_up)
- [Traversing the Tree: Stacks and Bitfields](#_traversing_the_tree_stacks_and_bitfields)
- [Traversing_the_Tree:_Stacks_and_Bitfields](#_traversing_the_tree_stacks_and_bitfields)
- [Implementation Challenges: The Memory Bottleneck](#_implementation_challenges_the_memory_bottleneck)
- [Implementation_Challenges:_The_Memory_Bottleneck](#_implementation_challenges_the_memory_bottleneck)
...
**Hierarchy Construction**: After sorting, each thread in a compute dispatch builds one part of the tree. By looking at the first bit where two adjacent Morton codes differ, a thread can determine where a node in the tree should be split. This is known as a **Linear BVH** (LBVH), and it allows the entire tree to be built in parallel without any global locks.
...
**Stack-Based**: Each thread maintains its own small array of "nodes to visit" in registers or local memory. This is fast but consumes precious registers, potentially lowering **Occupancy** (the number of active threads the hardware can run simultaneously).

**Stackless (Threaded Trees)**: Each node stores a "skip pointer" to the next node in a **Depth-First Search (DFS)** order. If a ray misses a node, it simply follows the skip pointer to bypass all that node’s children. This requires zero stack space but makes the tree-building process more complex.
...
struct Node { ... };
RWStructuredBuffer nodePool;
RWStructuredBuffer nodeCounter;
...
[Previous: Introduction to Advanced Data Structures](01_introduction.html) | [Next: Global Atomic Management](03_global_atomic_management.html)
