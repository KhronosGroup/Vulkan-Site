...
- **Component**: tutorial
- **Version**: latest
- **URL**: /tutorial/latest/ML_Inference/Building_the_Inference_Engine/02_hardcoded_forward_prop.html

## Table of Contents

- [The Network: MNIST Digit Recognition](#_the_network_mnist_digit_recognition)
- [The_Network:_MNIST_Digit_Recognition](#_the_network_mnist_digit_recognition)
- [The Binary Weight Format](#_the_binary_weight_format)
- [The_Binary_Weight_Format](#_the_binary_weight_format)
- [One Layer, Step by Step](#_one_layer_step_by_step)
- [One_Layer,_Step_by_Step](#_one_layer_step_by_step)
- [Buffers](#_buffers)
...
- [What This Teaches Us](#_what_this_teaches_us)
- [What_This_Teaches_Us](#_what_this_teaches_us)
...
The C++ code to read this is straightforward. You open the file, verify the magic number, read counts, read float arrays. See [weight_loader.hpp](../../_attachments/ml_inference/weight_loader.hpp) for the complete implementation—it’s about 50 lines total.
...
Recall from the [ML Inference Pipeline section](../ML_Inference_Pipeline/04_inference_explained.html) that a dense (fully connected) layer performs matrix multiplication—connecting every input to every output through learned weights. It’s one of the fundamental building blocks of neural networks.
...
Each thread computes one output element. It reads the entire input vector, multiplies by the corresponding row of the weight matrix, adds the bias, and applies ReLU activation. ReLU (Rectified Linear Unit, covered in [Neural Networks Fundamentals](../ML_Inference_Pipeline/03_neural_networks_fundamentals.html)) is just `max(0, x)`—it passes positive values unchanged and zeros out negative values, introducing the non-linearity that makes deep networks powerful.
...
Load weights from binary file into CPU memory

Create Vulkan buffers, upload weights

Dispatch layer 1 (784 → 128)

Barrier

Dispatch layer 2 (128 → 64)

Barrier

Dispatch layer 3 (64 → 10)
...
See [vulkan_mnist_inference.cpp](../../_attachments/ml_inference/vulkan_mnist_inference.cpp) for the complete implementation. The core inference loop is about 150 lines, most of it Vulkan boilerplate.
...
You train a model in PyTorch or TensorFlow, then export the weights. The training script is in [train_mnist.py](../../_attachments/ml_inference/train_mnist.py). It:

Defines the network architecture (matching what we hardcoded)

Trains on MNIST dataset for a few epochs

Exports learned weights to binary format
...
[Previous: Introduction](01_introduction.html) | [Next: From Hardcoded to Flexible](03_from_hardcoded_to_flexible.html)
