...
The Vulkan C++ headers also offer a C++ module. A large advantage of C++ modules is they give all the benefits of C++ without the overhead of long compile times. To use them, the .cppm file must be compiled for your target device.

|  | Module support in the Vulkan C++ headers is still experimental and might not work with all compilers. Unless you want to experiment, it’s recommended to **not** use them. |
| --- | --- |
...
recommend tinyobjloader library only for small not complex projects.
...
variable. Add VULKAN_SDK= there and Vulkan will be
