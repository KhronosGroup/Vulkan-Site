...
|  | In earlier chapters, the qraphics queue family index (`queueIndex`) was a local variable. Starting with this chapter we’ll need this index in multiple places so it has been made a member of the application class. |
| --- | --- |
...
The `vk::raii::CommandBuffers` constructor generates a `std::vector`, but we just want a single one here, so we move it out of that vector into a singular variable.
