...
- **Component**: tutorial
- **Version**: latest
- **URL**: /tutorial/latest/Advanced_glTF/Tooling_Production_Pipeline/02_blender_workflow.html

## Table of Contents

- [Naming Conventions: The Unwritten Contract](#_naming_conventions_the_unwritten_contract)
- [Naming_Conventions:_The_Unwritten_Contract](#_naming_conventions_the_unwritten_contract)
- [Vertex Groups and Skinning Weights](#_vertex_groups_and_skinning_weights)
- [Vertex_Groups_and_Skinning_Weights](#_vertex_groups_and_skinning_weights)
- [Custom Properties for Physics Extras](#_custom_properties_for_physics_extras)
- [Custom_Properties_for_Physics_Extras](#_custom_properties_for_physics_extras)
- [glTF Export Checklist](#_gltf_export_checklist)
- [glTF_Export_Checklist](#_gltf_export_checklist)
- [Morph Target (Shape Key) Workflow](#_morph_target_shape_key_workflow)
- [Morph_Target_(Shape_Key)_Workflow](#_morph_target_shape_key_workflow)
...
**Creation**: Create Shape Keys in the `Mesh Data` properties panel. The first key is always the `Basis` (the rest pose). Additional keys define displacements from that basis.

**Naming**: Give your shape keys semantic names (e.g., `blink_l`, `smile`, `mouth_open`). These names will be exported in the glTF `extras.targetNames` field, allowing your engine to address them by name.

**Range**: Ensure your shape keys are designed to be additive. If a vertex is moved by both a `smile` morph and a `blink` morph, the engine will sum their displacements.

**Export**: As noted in the checklist, ensure the **Shape Keys** and **Shape Key Animation** options are enabled in the glTF exporter.

**Validation**: After exporting, run your `.glb` through the **glTF-Validator**. It will check that your shape keys use **Sparse Accessors** where appropriate, which is a critical optimization for complex facial rigs.

[Previous: Introduction](01_introduction.html) | [Next: Validation](03_validation.html)
