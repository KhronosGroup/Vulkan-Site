...
- **Component**: tutorial
- **Version**: latest
- **URL**: /tutorial/latest/ML_Inference/Building_the_Inference_Engine/04_model_representation.html

## Table of Contents

- [What’s in a Graph?](#_whats_in_a_graph)
- [What’s_in_a_Graph?](#_whats_in_a_graph)
- [Representing Tensors](#_representing_tensors)
- [Representing Operations](#_representing_operations)
- [The Graph Container](#_the_graph_container)
- [The_Graph_Container](#_the_graph_container)
- [Building a Graph Manually](#_building_a_graph_manually)
- [Building_a_Graph_Manually](#_building_a_graph_manually)
- [Execution Order](#_execution_order)
...
class Tensor {
public:
    Tensor(std::string name, std::vector shape)
    ...
    void setData(std::vector data) { data_ = std::move(data); }
...
private:
    std::string name_;
    std::vector shape_;
    std::optional> data_;  // CPU data for constants
    std::optional buffer_;  // GPU buffer when allocated
};
...
public:
    ...
    // Connections
    void addInput(std::shared_ptr tensor) { inputs_.push_back(tensor); }
    void addOutput(std::shared_ptr tensor) { outputs_.push_back(tensor); }
...
public:
    std::shared_ptr addTensor(
        std::string name,
        std::vector shape
    ) {
        auto tensor = std::make_shared(name, shape);
        tensors_[name] = tensor;
        return tensor;
    }

    std::shared_ptr addOperation(std::string name, OpType type) {
        auto op = std::make_shared(name, type);
...
private:
    std::unordered_map> tensors_;
    std::vector> operations_;
    std::vector> inputs_;
    std::vector> outputs_;
};
...
std::vector> computeExecutionOrder(
    const ComputationGraph& graph
) {
    std::vector> ordered;
    std::unordered_set executed;

    // Keep trying to execute operations until all are done
    while (ordered.size() inputs()) {
...
[Previous: From Hardcoded to Flexible](03_from_hardcoded_to_flexible.html) | [Next: Model Formats as Input](05_model_formats_as_input.html)
