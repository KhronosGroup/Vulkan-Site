...
- **Component**: tutorial
- **Version**: latest
- **URL**: /tutorial/latest/Advanced_glTF/Morph_Targets_Facial_Animation/03_bindless_morph_buffers.html

## Table of Contents

- [Why Bindless?](#_why_bindless)
- [Enabling Device Features](#_enabling_device_features)
- [Enabling_Device_Features](#_enabling_device_features)
- [Setting Up the Descriptor Infrastructure](#_setting_up_the_descriptor_infrastructure)
- [Setting_Up_the_Descriptor_Infrastructure](#_setting_up_the_descriptor_infrastructure)
- [Creating the Morph Compute Pipeline](#_creating_the_morph_compute_pipeline)
- [Creating_the_Morph_Compute_Pipeline](#_creating_the_morph_compute_pipeline)
- [Morph Target Animation](#_morph_target_animation)
- [Morph_Target_Animation](#_morph_target_animation)
- [The Extended Compute Shader](#_the_extended_compute_shader)
- [The_Extended_Compute_Shader](#_the_extended_compute_shader)
- [Integrating with the Dispatch Loop](#_integrating_with_the_dispatch_loop)
- [Integrating_with_the_Dispatch_Loop](#_integrating_with_the_dispatch_loop)
- [Morph Targets and Skeletal Rigs Together](#_morph_targets_and_skeletal_rigs_together)
- [Morph_Targets_and_Skeletal_Rigs_Together](#_morph_targets_and_skeletal_rigs_together)
...
void update_morph_descriptors(
    ...
    const std::vector& morph_buffers)
{
    ...
    vkUpdateDescriptorSets(device, static_cast(writes.size()), writes.data(), 0, nullptr);
}
...
{
    // 1. Pipeline Layout
    std::vector layouts = { base_layout, morph_layout };
    ...
    layout_info.setLayoutCount = static_cast(layouts.size());
...
[[vk::binding(0, 0)]] StructuredBuffer    base_vertices;
[[vk::binding(1, 0)]] RWStructuredBuffer output_vertices;
[[vk::binding(2, 0)]] StructuredBuffer       joint_matrices;
[[vk::binding(3, 0)]] StructuredBuffer          joint_indices;
[[vk::binding(4, 0)]] StructuredBuffer         joint_weights;
...
[[vk::binding(0, 1)]] StructuredBuffer morph_targets[];
...
**Update weights.** The animation system advances the `WEIGHTS` animation channel and writes new float values into the per-mesh morph weight array.

**Cull inactive targets.** Any morph target with weight below a threshold (typically 0.005) is excluded from the dispatch. Build the compact `morph_indices` and `weights` arrays from only the active targets.

**Push constants.** Fill the `PushConstants` struct with the active target count, the descriptor indices of the active buffers, and their weights.

**Dispatch.** Bind the descriptor set (which has all morph target buffers registered), push constants, and dispatch the compute shader. One thread per vertex.

**Barrier.** Issue a `VkBufferMemoryBarrier2` on the output buffer (same as Chapter 3) before the rasterizer or ray tracing pass reads from it.
...
[Previous: Shape Key Ingestion](02_shape_key_ingestion.html) | [Next: Conclusion](04_conclusion.html)
