...
Although it is possible to create a staging image for this purpose, Vulkan also allows you to directly copy pixels from a buffer to an image. That’s less verbose, less limited and usually [faster](https://developer.nvidia.com/vulkan-memory-management).
...
* 
`vk::ImageLayout::eTransferSrcOptimal`: Optimal as source in a transfer operation, like `vk::raii::CommandBuffer::copyImageToBuffer`

* 
`vk::ImageLayout::eTransferDstOptimal`: Optimal as destination in a transfer operation, like `vk::raii::CommandBuffer::copyBufferToImage`
...
The automatically typed return values are a vk::raii::Buffer and a vk::raii::DeviceMemory, respectively.
...
The `tiling` specifies how texels are arranged in memory. Vulkan supports two fundamentally different modes for this:
...
|  | Linear tiled images are very limited. They e.g. may only work for 2D images, can’t be used for depth/stencil, can’t have multiple mip levels or layers. GPU access is also much slower than for optimal tiled images. So there are very little use cases for them. |
| --- | --- |
...
One example, however, would be if you wanted to use a linear tiled image as a staging image.
...
If you are using the barrier to transfer queue family ownership, then `srcQueueFamilyIndex` and `dstQueueFamilyIndex` fields should be the indices of the queue families.
They must be set to `vk::QueueFamilyIgnored` if you don’t want to do this (not the default value!).
...
The first parameter specifies in which pipeline stage the operations occur that should happen before the barrier.
...
The third parameter is either `{}` or `vk::DependencyFlagBits::eByRegion`.
...
The last three parameters reference arrays of pipeline barriers of the three available types: memory barriers, buffer memory barriers, and image memory barriers like the one we’re using here.
...
Buffer to image copy operations are enqueued using the `vk::raii::CommandBuffer::copyBufferToImage` function:

commandBuffer.copyBufferToImage(buffer, image, vk::ImageLayout::eTransferDstOptimal, region);

The third parameter indicates which layout the image is currently using.
...
Finally, the single-time command buffer can be ended.
...
It should be noted that `vk::PipelineStageFlagBits::eTransfer` is not a *real* stage within the graphics and compute pipelines.
