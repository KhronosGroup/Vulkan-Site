...
- **Component**: tutorial
- **Version**: latest
- **URL**: /tutorial/latest/Advanced_glTF/Debugging_Visual_Auditing/05_conclusion.html

## Table of Contents

- [What We Built in This Chapter](#_what_we_built_in_this_chapter)
- [What_We_Built_in_This_Chapter](#_what_we_built_in_this_chapter)
- [The Series in Review](#_the_series_in_review)
- [The_Series_in_Review](#_the_series_in_review)
...
**Flush Behavior**: Ensure that the `DebugDrawer` correctly clears its line buffer after every `flush()` call. If lines persist from previous frames, the buffer is not being cleared.

**Coordinate Space**: Verify that your debug lines appear in world space. A common error is passing local-space coordinates to the drawer, which will result in debug geometry following the character incorrectly.

**RenderDoc Inspection**: Use the **Buffer Viewer** in RenderDoc to inspect your compute skinning output buffer. If the values in the buffer match your expectations but the character is not rendering correctly, the problem is likely in your vertex input description or graphics pipeline state.

[Previous: RenderDoc Analysis](04_renderdoc_analysis.html)
