...
- **Component**: tutorial
- **Version**: latest
- **URL**: /tutorial/latest/Advanced_glTF/Morph_Targets_Facial_Animation/04_conclusion.html

## Table of Contents

- [What We Built](#_what_we_built)
- [What_We_Built](#_what_we_built)
- [Tradeoffs to Keep in Mind](#_tradeoffs_to_keep_in_mind)
- [Tradeoffs_to_Keep_in_Mind](#_tradeoffs_to_keep_in_mind)
...
**Sparse Accessor Expansion**: Verify that vertices with no displacement in a particular morph target correctly remain at their base positions.

**Bindless Indexing**: Use a GPU debugger (like RenderDoc) to inspect the `morphTargets[]` array. Ensure that the descriptor indices passed in `PushConstants` match the correct buffer slots.

**Order of Operations**: Ensure that morph displacements are applied **before** skeletal skinning. If they are applied after, facial features will appear to "float" or detach when the head rotates.

[Previous: Bindless Morph Buffers](03_bindless_morph_buffers.html) | [Next: Tooling & The Production Pipeline](../Tooling_Production_Pipeline/01_introduction.html)
