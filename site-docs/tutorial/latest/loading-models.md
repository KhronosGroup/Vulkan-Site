...
- [Library](#_library)
- [Updating the fragment shader](#_updating_the_fragment_shader)
- [Updating_the_fragment_shader](#_updating_the_fragment_shader)
...
This was mentioned in the [Development Environment](02_Development_environment.html) chapter and should be part of the dependencies for this portion of the tutorial.

Back in the [combined image sampler chapter](06_Texture_mapping/02_Combined_image_sampler.html), we multiplied the sampled texture color by the interpolated per-vertex color to demonstrate that both were being combined:
...
The somewhat complex construction `vk::IndexTypeValue::value` automatically gets you the indices' type.

commandBuffer.bindIndexBuffer(*indexBuffer, 0, vk::IndexTypeValue::value);
...
for (const auto& shape : shapes)
{
    for (const auto& index : shape.mesh.indices)
    {
        ...
        ...
				...
				auto [it, inserted] = uniqueVertices.insert({vertex, static_cast(vertices.size())});
				if (inserted)
				{
            vertices.push_back(vertex);
				}
...
namespace std
{
    template struct hash
    {
        ...
        {
            return ((hash()(vertex.pos) ^ (hash()(vertex.color) > 1) ^ (hash()(vertex.texCoord) 
