...
- **Component**: tutorial
- **Version**: latest
- **URL**: /tutorial/latest/Advanced_glTF/Physics_Integration/02_bone_proxy_colliders.html

## Table of Contents

- [Why Not Use the Real Mesh?](#_why_not_use_the_real_mesh)
- [Why_Not_Use_the_Real_Mesh?](#_why_not_use_the_real_mesh)
- [Choosing the Right Shape](#_choosing_the_right_shape)
- [Choosing_the_Right_Shape](#_choosing_the_right_shape)
- [Defining Collider Properties in glTF Extras](#_defining_collider_properties_in_gltf_extras)
- [Defining_Collider_Properties_in_glTF_Extras](#_defining_collider_properties_in_gltf_extras)
- [Creating the Physics Bodies](#_creating_the_physics_bodies)
- [Creating_the_Physics_Bodies](#_creating_the_physics_bodies)
- [Updating Kinematic Bodies Each Frame](#_updating_kinematic_bodies_each_frame)
- [Updating_Kinematic_Bodies_Each_Frame](#_updating_kinematic_bodies_each_frame)
...
ColliderDef parse_collider_extras(const tinygltf::Value& extras)
{
    ...
    const std::string shape_str = phys.Get("collider").Get();
    ...
    if (phys.Has("radius"))       def.radius       = static_cast(phys.Get("radius").GetNumberAsDouble());
    if (phys.Has("half_height"))  def.half_height  = static_cast(phys.Get("half_height").GetNumberAsDouble());
    if (phys.Has("mass"))         def.mass         = static_cast(phys.Get("mass").GetNumberAsDouble());

    if (phys.Has("box_half_extents")) {
        const auto& ext = phys.Get("box_half_extents");
        def.box_half_extents = {
            static_cast(ext.Get(0).GetNumberAsDouble()),
            static_cast(ext.Get(1).GetNumberAsDouble()),
            static_cast(ext.Get(2).GetNumberAsDouble())
        };
    }

    if (phys.Has("collision_group")) def.collision_group = phys.Get("collision_group").Get();
    if (phys.Has("collision_mask"))  def.collision_mask  = phys.Get("collision_mask").Get();
    ...
    PhysicsWorld& physics_world)
{
    ...
    for (uint32_t joint_idx = 0; joint_idx  shape;
...
[Previous: Introduction](01_introduction.html) | [Next: Constraints & Joint Limits](03_constraints_and_joint_limits.html)
