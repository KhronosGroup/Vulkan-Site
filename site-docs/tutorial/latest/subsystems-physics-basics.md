...
- **Component**: tutorial
- **Version**: latest
- **URL**: /tutorial/latest/Building_a_Simple_Engine/Subsystems/04_physics_basics.html

## Table of Contents

- [Physics System Fundamentals](#_physics_system_fundamentals)
- [Physics_System_Fundamentals](#_physics_system_fundamentals)
- [Physics System Architecture](#_physics_system_architecture)
- [Physics_System_Architecture](#_physics_system_architecture)
- [Basic Physics System Implementation](#_basic_physics_system_implementation)
- [Basic_Physics_System_Implementation](#_basic_physics_system_implementation)
- [Integrating with the Engine](#_integrating_with_the_engine)
- [Integrating_with_the_Engine](#_integrating_with_the_engine)
- [Basic Implementation of Physics Simulation](#_basic_implementation_of_physics_simulation)
- [Basic_Implementation_of_Physics_Simulation](#_basic_implementation_of_physics_simulation)
- [Basic Usage Example](#_basic_usage_example)
- [Basic_Usage_Example](#_basic_usage_example)
- [Limitations of Basic Physics Systems](#_limitations_of_basic_physics_systems)
- [Limitations_of_Basic_Physics_Systems](#_limitations_of_basic_physics_systems)
...
public:
    ...
    // Collider management
    void SetCollider(std::shared_ptr collider) { m_Collider = collider; }
    std::shared_ptr GetCollider() const { return m_Collider; }
...
struct CollisionInfo {
    std::shared_ptr bodyA;
    std::shared_ptr bodyB;
...
public:
    ...
    std::shared_ptr CreateRigidBody();
    void DestroyRigidBody(std::shared_ptr body);
...
private:
    ...
    void DetectCollisions(std::vector& collisions);
    void ResolveCollisions(std::vector& collisions);
...
void PhysicsSystem::DetectCollisions(std::vector& collisions) {
    // Simple O(n²) collision detection
    for (size_t i = 0; i IsKinematic() && bodyB->IsKinematic()) {
                continue;
            }
...
void PhysicsSystem::ResolveCollisions(std::vector& collisions) {
...
bool PhysicsSystem::SphereVsSphere(const RigidBody& bodyA, const RigidBody& bodyB, CollisionInfo& info) {
    auto sphereA = std::static_pointer_cast(bodyA.GetCollider());
    auto sphereB = std::static_pointer_cast(bodyB.GetCollider());
...
void Game::Initialize() {
    ...
    auto groundCollider = std::make_shared(glm::vec3(50.0f, 1.0f, 50.0f));
    ...
    box->SetMass(1.0f);
    auto boxCollider = std::make_shared(glm::vec3(0.5f, 0.5f, 0.5f));
    ...
    sphere->SetMass(2.0f);
    auto sphereCollider = std::make_shared(0.7f);
...
void Game::OnExplosion(const glm::vec3& position, float force) {
    ...
    for (auto& physicsObject : m_PhysicsObjects) {
        if (!physicsObject->IsKinematic()) {
            ...
            if (distance ApplyImpulse(direction * impulseMagnitude);
            }
        }
    }
}
