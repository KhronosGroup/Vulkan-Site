...
- **Component**: tutorial
- **Version**: latest
- **URL**: /tutorial/latest/Advanced_Vulkan_Compute/09_Specialized_Math/02_cooperative_matrices.html

## Table of Contents

- [The Concept: Matrix Fragments and Subgroup Scope](#_the_concept_matrix_fragments_and_subgroup_scope)
- [The_Concept:_Matrix_Fragments_and_Subgroup_Scope](#_the_concept_matrix_fragments_and_subgroup_scope)
- [Memory Layout: Strides and Majorness](#_memory_layout_strides_and_majorness)
- [Memory_Layout:_Strides_and_Majorness](#_memory_layout_strides_and_majorness)
- [Slang: Tiled Matrix Multiplication](#_slang_tiled_matrix_multiplication)
- [Slang:_Tiled_Matrix_Multiplication](#_slang_tiled_matrix_multiplication)
- [GLSL: The Low-Level Win](#_glsl_the_low_level_win)
- [GLSL:_The_Low-Level_Win](#_glsl_the_low_level_win)
- [Hardware Constraints and Capabilities](#_hardware_constraints_and_capabilities)
- [Hardware_Constraints_and_Capabilities](#_hardware_constraints_and_capabilities)
...
**Row-Major vs. Column-Major**: Most Vulkan applications prefer **Row-Major** (where elements of a row are contiguous).

**Stride**: This is the distance (in elements, not bytes) between the start of one row and the start of the next. For a simple tightly-packed matrix, the stride is equal to the number of columns.
...
void computeMain(uint3 threadId : SV_GroupThreadID, uint3 groupId : SV_GroupID) {
    ...
    CooperativeMatrix acc = 0.0f;

    // Loop over the K dimension in blocks of 'K'
    for (uint32_t k = 0; k  matA;
        CooperativeMatrix matB;
...
[Previous: Introduction](01_introduction.html) | [Next: Mastering Mixed Precision](03_mixed_precision.html)
