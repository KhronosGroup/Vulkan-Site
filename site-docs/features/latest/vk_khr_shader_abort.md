...
This extension adds a reliable way to abort from a shader, with message passing when used in combination with [VK_KHR_device_fault](../../../../refpages/latest/refpages/source/VK_KHR_device_fault.html).
...
All of this can be encoded into a single SPIR-V operation, with message retrieval performed by [VK_KHR_device_fault](../../../../refpages/latest/refpages/source/VK_KHR_device_fault.html) when available.
...
Execution of `OpAbortKHR` will trigger device loss in the API within finite time, making the data in *Message* available via [vkGetDeviceFaultDebugInfoKHR](../../../../refpages/latest/refpages/source/vkGetDeviceFaultDebugInfoKHR.html) for further analysis.
...
If [VK_KHR_device_fault](../../../../refpages/latest/refpages/source/VK_KHR_device_fault.html) is supported and enabled, new structures to retrieve shader abort messages can be chained into [VkDeviceFaultDebugInfoKHR](../../../../refpages/latest/refpages/source/VkDeviceFaultDebugInfoKHR.html) are added:
...
`VkDeviceFaultShaderAbortMessageInfoKHR` can be included in the `pNext` chain of [VkDeviceFaultDebugInfoKHR](../../../../refpages/latest/refpages/source/VkDeviceFaultDebugInfoKHR.html) to retrieve  the total size of message data returned by the implementation and the actual the messages returned by `OpAbortKHR` instructions in shaders.
...
...
             OpDecorate %msg_array_t UTFEncodedKHR
             OpDecorate %msg_array_t ArrayStride 1
             OpDecorate %src_array_t UTFEncodedKHR
             OpMemberDecorate %abortMessageLoadType 0 Offset 0
             OpMemberDecorate %abortMessageLoadType 1 Offset 12
...
     %uint32_t = OpTypeInt 32 0
       %char_t = OpTypeInt 8 1
      %uint_12 = OpConstant %uint32_t 12
  %msg_array_t = OpTypeArray %char_t %uint_12
  %src_array_t = OpTypeArray %char_t %uint_12
           // "test: %u" encoded as 32-bit words
           %14 = OpConstantDataKHR %src_array_t 1953393013 1965367354 0
%abortMessageLoadType = OpTypeStruct %msg_array_t %uint
%abortMessageValueType = OpTypeStruct %src_array_t %uint
...
    %testval = OpConstant %uint32_t 65536
         %18 = OpCompositeConstruct %abortMessageValueType %14 %testval
               OpAbortKHR %abortMessageLoadType %18
