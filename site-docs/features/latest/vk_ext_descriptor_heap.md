...
This expands on [VK_EXT_descriptor_buffer](../../../../refpages/latest/refpages/source/VK_EXT_descriptor_buffer.html) to solve a number of identified issues with that extension.

[VK_EXT_descriptor_buffer](../../../../refpages/latest/refpages/source/VK_EXT_descriptor_buffer.html) simplified descriptor management, but several warts remained with that extension that would be useful to iron out:
...
* 
[VK_EXT_inline_uniform_block](../../../../refpages/latest/refpages/source/VK_EXT_inline_uniform_block.html) added one more (embedding constants in descriptor sets), but this method is not necessarily a universal fast path.
...
* 
Fully replace the functionality of [VK_EXT_descriptor_buffer](../../../../refpages/latest/refpages/source/VK_EXT_descriptor_buffer.html)
...
|  | While this extension makes constant reference to [VK_EXT_descriptor_buffer](../../../../refpages/latest/refpages/source/VK_EXT_descriptor_buffer.html), VK_EXT_descriptor_heap does not depend on it; VK_EXT_descriptor_heap is intended as a full replacement for that extension in newer hardware. |
| --- | --- |
...
This proposal assumes, but does not require, an understanding of [VK_EXT_descriptor_buffer](../../../../refpages/latest/refpages/source/VK_EXT_descriptor_buffer.html); it is recommended that you read the background information in its proposal document before reading this.

This extension requires [VK_KHR_buffer_device_address](../../../../refpages/latest/refpages/source/VK_KHR_buffer_device_address.html) or Vulkan 1.2, and [VK_KHR_shader_untyped_pointers](../../../../refpages/latest/refpages/source/VK_KHR_shader_untyped_pointers.html).
...
Unlike [vkGetDescriptorEXT](../../../../refpages/latest/refpages/source/vkGetDescriptorEXT.html), multiple descriptors can be written at once, allowing for more rapid execution.
When implementing [VK_EXT_descriptor_buffer](../../../../refpages/latest/refpages/source/VK_EXT_descriptor_buffer.html), Virtual Machine (VM) implementations noted that this was a high frequency call with immediate return needed; which meant significant latency going through the VM to the native driver for each call and waiting for the result.
...
However, the actual bit values and size of a descriptor written with this extension may differ from those obtained by [VK_EXT_descriptor_buffer](../../../../refpages/latest/refpages/source/VK_EXT_descriptor_buffer.html).
...
Rather than having "generic" looking descriptor buffers like [VK_EXT_descriptor_buffer](../../../../refpages/latest/refpages/source/VK_EXT_descriptor_buffer.html), there are explicitly two heaps - one for samplers, and one for other resources.
...
| --- | --- |
This mirrors the advice given in [VK_EXT_descriptor_buffer](../../../../refpages/latest/refpages/source/VK_EXT_descriptor_buffer.html) for [vkCmdBindDescriptorBuffersEXT](../../../../refpages/latest/refpages/source/vkCmdBindDescriptorBuffersEXT.html), or for heap bindings in DirectX® 12.
...
The implementation manages these bytes for internal descriptors needed to ensure correct operation of things like embedded samplers and fixed operations (e.g. [vkCmdBlitImage](../../../../refpages/latest/refpages/source/vkCmdBlitImage.html)).
...
When [VK_EXT_shader_object](../../../../refpages/latest/refpages/source/VK_EXT_shader_object.html) is supported, a shader create flag is similarly provided:
...
`vkCmdPushDataEXT` will invalidate, and be invalidated by, any state set by [vkCmdPushConstants](../../../../refpages/latest/refpages/source/vkCmdPushConstants.html), [vkCmdPushDescriptorSetKHR](../../../../refpages/latest/refpages/source/vkCmdPushDescriptorSet.html), or [vkCmdPushDescriptorSetWithTemplateKHR](../../../../refpages/latest/refpages/source/vkCmdPushDescriptorSetWithTemplate.html).
...
Unlike [VK_EXT_descriptor_buffer](../../../../refpages/latest/refpages/source/VK_EXT_descriptor_buffer.html), this extension does not add direct support for descriptor set layouts, and instead includes functionality to allow mapping descriptors with `DescriptorSet` and `Binding` decorations to heap resources.
...
`VkShaderDescriptorSetAndBindingMappingInfoEXT` can be chained to the `pNext` chain of [VkPipelineShaderStageCreateInfo](../../../../refpages/latest/refpages/source/VkPipelineShaderStageCreateInfo.html) or [VkShaderCreateInfoEXT](../../../../refpages/latest/refpages/source/VkShaderCreateInfoEXT.html) to indicate where resources with `DescriptorSet` and `Binding` decorations should be sourced from for that shader.
...
When the `nullDescriptor` feature added by [VK_EXT_robustness2](../../../../refpages/latest/refpages/source/VK_EXT_robustness2.html) is supported, null descriptors can be written by setting the corresponding element of `VkResourceDescriptorDataEXT` to `NULL` when writing a resource descriptor.

When the `customBorderColors` feature added by [VK_EXT_custom_border_color](../../../../refpages/latest/refpages/source/VK_EXT_custom_border_color.html) is used, applications using samplers with custom border colors must explicitly register and unregister border colors with the device:
...
Up to [`VkPhysicalDeviceCustomBorderColorPropertiesEXT::maxCustomBorderColorSamplers`](../../../../refpages/latest/refpages/source/VkPhysicalDeviceCustomBorderColorPropertiesEXT.html) border colors can be registered; if too many are already registered, `vkRegisterCustomBorderColorEXT` will return `VK_ERROR_TOO_MANY_OBJECTS`.
...
Additionally, tools must capture some data from the implementation during capture to give the implementation the information to recreate identical descriptors during replay, should use an identically created [VkDevice](../../../../refpages/latest/refpages/source/VkDevice.html) and [VkInstance](../../../../refpages/latest/refpages/source/VkInstance.html), and should create all captured descriptors before creating any others for use in the tool.
...
To try to get the same device address range for a buffer allocation, tools can use the [VkPhysicalDeviceBufferDeviceAddressFeatures::bufferDeviceAddressCaptureReplay](../../../../refpages/latest/refpages/source/VkPhysicalDeviceBufferDeviceAddressFeatures.html) feature to recreate the buffer and any memory it is bound to with opaque capture data.
[VkPhysicalDeviceBufferDeviceAddressFeatures::bufferDeviceAddressCaptureReplay](../../../../refpages/latest/refpages/source/VkPhysicalDeviceBufferDeviceAddressFeatures.html) must be supported if `descriptorHeapCaptureReplay` is supported.
...
Memory bound to the image during replay must match the memory bound during capture, with memory objects recreated with identical parameters other than including the opaque capture data in [VkDeviceMemoryOpaqueCaptureAddressInfo](../../../../refpages/latest/refpages/source/VkDeviceMemoryOpaqueCaptureAddressInfo.html) which was initially captured with [vkGetDeviceMemoryOpaqueCaptureAddress](../../../../refpages/latest/refpages/source/vkGetDeviceMemoryOpaqueCaptureAddress.html).
...
An image with this flag can be recreated from a previously captured image by passing `data` back into image creation by chaining the following structure to [VkImageCreateInfo](../../../../refpages/latest/refpages/source/VkImageCreateInfo.html), with all other creation parameters matching:
...
If the implementation is unable to recreate an identical image from this opaque data that would result in the same descriptors, [vkCreateImage](../../../../refpages/latest/refpages/source/vkCreateImage.html) must return `VK_ERROR_INVALID_OPAQUE_CAPTURE_ADDRESS`.
...
Memory bound to the tensor during replay must match the memory bound during capture, with memory objects recreated with identical parameters other than including the opaque capture data in [VkDeviceMemoryOpaqueCaptureAddressInfo](../../../../refpages/latest/refpages/source/VkDeviceMemoryOpaqueCaptureAddressInfo.html) which was initially captured with [vkGetDeviceMemoryOpaqueCaptureAddress](../../../../refpages/latest/refpages/source/vkGetDeviceMemoryOpaqueCaptureAddress.html).
...
A tensor with this flag can be recreated from a previously captured tensor by passing `data` back into tensor creation by chaining `VkOpaqueCaptureDataCreateInfoEXT` to [VkTensorCreateInfoARM](../../../../refpages/latest/refpages/source/VkTensorCreateInfoARM.html), with all other creation parameters matching.

If the implementation is unable to recreate an identical tensor from this opaque data that would result in the same descriptors, [vkCreateTensorARM](../../../../refpages/latest/refpages/source/vkCreateTensorARM.html) must return `VK_ERROR_INVALID_OPAQUE_CAPTURE_ADDRESS`.
...
`VkIndirectCommandsLayoutPushDataTokenNV` can be included in the `pNext` chain of [VkIndirectCommandsLayoutTokenNV](../../../../refpages/latest/refpages/source/VkIndirectCommandsLayoutTokenNV.html) when the `VK_INDIRECT_COMMANDS_TOKEN_TYPE_PUSH_DATA_NV` token type is specified to enable the use of this token.
...
This structure can be included in the `pNext` chain of [VkImageFormatProperties2](../../../../refpages/latest/refpages/source/VkImageFormatProperties2.html) to query the number of image descriptors required for subsampled images.
...
If the `descriptorHeap` feature is enabled, [VK_AMD_shader_fragment_mask](../../../../refpages/latest/refpages/source/VK_AMD_shader_fragment_mask.html) must not be enabled.
...
| samplerYcbcrConversionCount | 3 | max | [combinedImageSamplerDescriptorCount](../../../../refpages/latest/refpages/source/VkSamplerYcbcrConversionImageFormatProperties.html) |
...
For example, when using the [VK_EXT_descriptor_buffer](../../../../refpages/latest/refpages/source/VK_EXT_descriptor_buffer.html) extension, vkd3d-proton packs storage buffers and texel buffers together where possible to emulate atomic counters in HLSL, which can specify their counter payload in a separate address.
...
As this extension allows the creation of descriptors without ever creating a sampler, image view, or buffer view object, in order to allow naming the resulting descriptors, [VkDebugUtilsObjectNameInfoEXT](../../../../refpages/latest/refpages/source/VkDebugUtilsObjectNameInfoEXT.html) can now be included in the `pNext` chain of `VkSamplerCreateInfo` and `VkResourceDescriptorInfoEXT` when either writing a descriptor or creating an embedded sampler, which associates a static name with the written descriptor.
...
With the existing descriptor set layout interface, applications need to specify the push constants they are using in each shader stage with [VkPipelineLayoutCreateInfo](../../../../refpages/latest/refpages/source/VkPipelineLayoutCreateInfo.html).
...
A number of image descriptors will be consumed for each such resource, according to [VkSamplerYcbcrConversionImageFormatProperties::combinedImageSamplerDescriptorCount](../../../../refpages/latest/refpages/source/VkSamplerYcbcrConversionImageFormatProperties.html).
...
They are largely the same as [VK_EXT_descriptor_indexing](../../../../refpages/latest/refpages/source/VK_EXT_descriptor_indexing.html) with all features enabled.
For any shader declaring the `DescriptorHeapEXT` capability, it is no longer necessary to specify the `NonUniform` decoration.
