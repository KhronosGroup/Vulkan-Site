...
- [5. Issues](#_issues)
- [5.1. How is hit kind specified when recording a hit object from a ray query?](#_how_is_hit_kind_specified_when_recording_a_hit_object_from_a_ray_query)
- [5.1._How_is_hit_kind_specified_when_recording_a_hit_object_from_a_ray_query?](#_how_is_hit_kind_specified_when_recording_a_hit_object_from_a_ray_query)
...
The initial `hitObjectRecordFromQueryEXT` overload does not take a hit kind.
For custom intersection geometry, the shader must be able to encode the hit
kind in the hit object.
Version 2 adds an overload with an additional `uint hitKind` parameter.
If the committed intersection in the ray query is triangle geometry, `hitKind`
is ignored and the value from the ray query is used instead.
