...
Another problem is that currently, applications are unable to align the renderArea with tile boundaries which would achieve the most efficient rendering. The command [vkGetRenderAreaGranularity](../../../../refpages/latest/refpages/source/vkGetRenderAreaGranularity.html) does not allow implementations to fully describe the tiling grid and reported granularity is based solely on a renderpass.
...
The reported value for `apronSize` is described in [VK_QCOM_tile_shading](../../../../refpages/latest/refpages/source/VK_QCOM_tile_shading.html).
