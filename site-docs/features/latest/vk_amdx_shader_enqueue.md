...
The `VkPipelineShaderStageNodeCreateInfoAMDX` provides options for specifying how the shader is specified with regards to its entry point name and index, and can be chained to the [VkPipelineShaderStageCreateInfo](../../../../refpages/latest/refpages/source/VkPipelineShaderStageCreateInfo.html) structure.
...
When creating such a graphics pipeline from libraries as an interaction with [VK_EXT_graphics_pipeline_library](../../../../refpages/latest/refpages/source/VK_EXT_graphics_pipeline_library.html), those libraries must also have been created with those flags.
...
The values of `MaxNumWorkgroupsAMDX` and `StaticNumWorkgroupsAMDX` must be less than or equal to `[VkPhysicalDeviceShaderEnqueuePropertiesAMDX](../../../../refpages/latest/refpages/source/VkPhysicalDeviceShaderEnqueuePropertiesAMDX.html)::maxExecutionGraphWorkgroupCount`.

The product of the X, Y, and Z values of `MaxNumWorkgroupsAMDX` and `StaticNumWorkgroupsAMDX` must be less than or equal to `[VkPhysicalDeviceShaderEnqueuePropertiesAMDX](../../../../refpages/latest/refpages/source/VkPhysicalDeviceShaderEnqueuePropertiesAMDX.html)::maxExecutionGraphWorkgroups`.
