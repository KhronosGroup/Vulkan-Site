...
- **Component**: features
- **Version**: latest
- **URL**: /features/latest/features/proposals/VK_NV_cooperative_vector.html
...
There are also functions to load/store vectors from memory:
    ...
    void coopVecLoadNV(out coopvecNV v, volatile coherent ArrayElemTy[] buf, uint offset);
...
Example showing a 2x32 MLP evaluation in GLSL:
    ...
    coopvecNV inputVec = coopvecNV(float16_t(materialstate), float16_t(shininess), ... );
    ...
    // ReLU activation
    mlpVec = max(coopvecNV(float16_t(0)), mlpVec);
