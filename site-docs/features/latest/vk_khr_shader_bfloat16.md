...
`shaderBFloat16CooperativeMatrix` must be `VK_FALSE` if [VK_KHR_cooperative_matrix](../../../../refpages/latest/refpages/source/VK_KHR_cooperative_matrix.html) is not supported.
At least one of `shaderBFloat16DotProduct` and `shaderBFloat16CooperativeMatrix` must be supported.

A new [VkComponentTypeKHR](../../../../refpages/latest/refpages/source/VkComponentTypeKHR.html) is added that can be reported as supported by [vkGetPhysicalDeviceCooperativeMatrixPropertiesKHR](../../../../refpages/latest/refpages/source/vkGetPhysicalDeviceCooperativeMatrixPropertiesKHR.html):
...
If `shaderBFloat16CooperativeMatrix` is supported, at least one entry in [vkGetPhysicalDeviceCooperativeMatrixPropertiesKHR::pProperties](../../../../refpages/latest/refpages/source/vkGetPhysicalDeviceCooperativeMatrixPropertiesKHR.html) must include this type in all of its `AType`, `BType`, and `CType` members.
