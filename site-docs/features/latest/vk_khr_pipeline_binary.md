...
A new [VkResult](../../../../refpages/latest/refpages/source/VkResult.html) value is added so that `vkCreatePipelineBinariesKHR` can indicate that an implementation supporting `pipelineBinaryInternalCache` [Properties](#properties) does not have a binary in its internal cache:

VK_PIPELINE_BINARY_MISSING_KHR = 1000483000

A new [VkResult](../../../../refpages/latest/refpages/source/VkResult.html) value is added so that `vkGetPipelineBinaryDataKHR` can indicate that the application has not provided enough storage to write pipeline binary data into:

VK_ERROR_NOT_ENOUGH_SPACE_KHR = -1000483000

A new [VkPipelineCreateFlagBits2KHR](../../../../refpages/latest/refpages/source/VkPipelineCreateFlagBits2.html) value is required to be able to obtain binary data from a pipeline object via this extension after creation:
...
It is the application’s responsibility to ensure the pipeline create info in this call exactly matches the pipeline create info of the pipeline used to create the key/binary pairs, other than the inclusion of this structure and any shader modules that were declared in `VkPipelineShaderStageCreateInfo` instances at key generation time as they will be ignored by the implementation when creating a pipeline from binaries. Shader module identifiers and modules declared by `VkShaderModuleCreateInfo` instances will also be ignored.
