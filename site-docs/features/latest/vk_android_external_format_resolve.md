...
When this resolve mode is specified, the `resolveImageView` member of [VkRenderingAttachmentInfo](../../../../refpages/latest/refpages/source/VkRenderingAttachmentInfo.html) used as a color attachment must be set to a `VkImageView` created with an image with a non-zero external format.
...
The `layerCount` and `colorAttachmentCount` members of [VkRenderingInfo](../../../../refpages/latest/refpages/source/VkRenderingInfo.html) must be 1.

The `viewMask` member of [VkRenderingInfo](../../../../refpages/latest/refpages/source/VkRenderingInfo.html) must be 0.
...
Implementations may need to know that an external format YCBCR format is being used when creating a pipeline, so when dynamic rendering is used, [VkExternalFormatANDROID](../../../../refpages/latest/refpages/source/VkExternalFormatANDROID.html) can be chained to `VkGraphicsPipelineCreateInfo`, indicating the format of the resolve image.
...
Graphics pipelines that include a [VkExternalFormatANDROID](../../../../refpages/latest/refpages/source/VkExternalFormatANDROID.html) structure with a non-zero value must only write to a single color attachment, must not export depth or stencil from the fragment shader, and must disable blending.

When dynamic rendering is used with secondary command buffer inheritance, the external format **must** be made known to the secondary command buffers by including [VkExternalFormatANDROID](../../../../refpages/latest/refpages/source/VkExternalFormatANDROID.html) in the `pNext` chain of [VkCommandBufferInheritanceInfo](../../../../refpages/latest/refpages/source/VkCommandBufferInheritanceInfo.html), and the external format must match that in the render pass instance.
...
These can only be used with [VkSubpassDescription2](../../../../refpages/latest/refpages/source/VkSubpassDescription2.html), and similar restrictions apply to this as they do to dynamic rendering:

* 
The resolve attachment must be an attachment that has a format of `VK_FORMAT_UNDEFINED` and includes a [VkExternalFormatANDROID](../../../../refpages/latest/refpages/source/VkExternalFormatANDROID.html) structure in its `pNext` chain
...
Using an external format image as an input attachment is only valid when the feature bits queried via [vkGetAndroidHardwareBufferPropertiesANDROID](../../../../refpages/latest/refpages/source/vkGetAndroidHardwareBufferPropertiesANDROID.html) advertise this functionality.
...
If the implementation supports this property, an external format image can be used as an input attachment without the typically required feature bits advertised by [vkGetAndroidHardwareBufferPropertiesANDROID](../../../../refpages/latest/refpages/source/vkGetAndroidHardwareBufferPropertiesANDROID.html).
...
Applications must import depth images with [VkFormat](../../../../refpages/latest/refpages/source/VkFormat.html) in order to render to them.
