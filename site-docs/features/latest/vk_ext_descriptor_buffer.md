...
[VK_VALVE_mutable_descriptor_type](../../../../refpages/latest/refpages/source/VK_VALVE_mutable_descriptor_type.html) includes support for option 1,
...
As [VK_EXT_descriptor_indexing](../../../../refpages/latest/refpages/source/VK_EXT_descriptor_indexing.html) is required, its descriptor limits apply.
...
These calls are invariant across the lifetime of the device, and between [VkDevice](../../../../refpages/latest/refpages/source/VkDevice.html) objects created from the same physical device(s), with the same creation parameters.
...
Layouts created with this flag must not be used to create a [VkDescriptorSet](../../../../refpages/latest/refpages/source/VkDescriptorSet.html) and must not include dynamic uniform buffers or dynamic storage buffers.
...
A notable change here is that there is no longer any need for [VkBufferView](../../../../refpages/latest/refpages/source/VkBufferView.html) objects.
...
If the `descriptorBufferImageLayoutIgnored` feature is enabled, the `imageLayout` in [VkDescriptorImageInfo](../../../../refpages/latest/refpages/source/VkDescriptorImageInfo.html) is ignored, otherwise it specifies the layout that the descriptor will be used with.
...
In addition, [vkGetDeviceMemoryOpaqueCaptureAddress](../../../../refpages/latest/refpages/source/vkGetDeviceMemoryOpaqueCaptureAddress.html) must be used to capture the opaque address and replay it with [VkMemoryOpaqueCaptureAddressAllocateInfo](../../../../refpages/latest/refpages/source/VkMemoryOpaqueCaptureAddressAllocateInfo.html), for any memory used by resources with these handles.
...
If the `descriptorBuffer` feature is enabled, [VK_AMD_shader_fragment_mask](../../../../refpages/latest/refpages/source/VK_AMD_shader_fragment_mask.html) must not be enabled.
...
If [VK_VALVE_mutable_descriptor_type](../../../../refpages/latest/refpages/source/VK_VALVE_mutable_descriptor_type.html) is used,
...
* 
`combinedImageSamplerDensityMapDescriptorSize` describes the size, in bytes, of a VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER descriptor when using the VK_SAMPLER_CREATE_SUBSAMPLED_BIT_EXT flag of the [VK_EXT_fragment_density_map](../../../../refpages/latest/refpages/source/VK_EXT_fragment_density_map.html) extension.
...
If [VK_VALVE_mutable_descriptor_type](../../../../refpages/latest/refpages/source/VK_VALVE_mutable_descriptor_type.html) is supported, we only need one descriptor set layout which supports all descriptor types for the heap type.
