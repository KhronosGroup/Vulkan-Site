...
|  | This extension is a direct promotion of [VK_VALVE_mutable_descriptor_type](../../../../refpages/latest/refpages/source/VK_VALVE_mutable_descriptor_type.html). As that extension already shipped before proposal documents existed, this document has been written retroactively during promotion to EXT. |
| --- | --- |
...
Typically when specifying a [descriptor set layout binding](../../../../refpages/latest/refpages/source/VkDescriptorSetLayoutBinding.html), applications have to choose one of the available [descriptor types](../../../../refpages/latest/refpages/source/VkDescriptorType.html) that will occupy that binding.
...
`VkMutableDescriptorTypeCreateInfoEXT` can be added to the `pNext` chain of [VkDescriptorSetLayoutCreateInfo](../../../../refpages/latest/refpages/source/VkDescriptorSetLayoutCreateInfo.html), where each entry in `pMutableDescriptorTypeLists` corresponds to a binding at the same index in `pBindings`.
...
It is possible to go beyond the minimum supported set. For this purpose, the desired descriptor set layout can be queried with [vkGetDescriptorSetLayoutSupport](../../../../refpages/latest/refpages/source/vkGetDescriptorSetLayoutSupport.html).
...
Host-only descriptor sets cannot be bound to a command buffer, and their contents must be copied to a non-host-only set using [vkUpdateDescriptorSets](../../../../refpages/latest/refpages/source/vkUpdateDescriptorSets.html) before those descriptors can be used.

Descriptor pools are specified as host-only using a new [create flag](../../../../refpages/latest/refpages/source/VkDescriptorSetLayoutCreateFlagBits.html):

VK_DESCRIPTOR_POOL_CREATE_HOST_ONLY_BIT_EXT = 0x00000004

Any descriptor set created from a pool with this flag set is a host-only descriptor set.

The memory layout of a descriptor set may also be optimized for device access rather than host access, so a new [create flag](../../../../refpages/latest/refpages/source/VkDescriptorSetLayoutCreateFlagBits.html) is provided to specify when a layout will be used with a host-only pool:
...
In addition, as such layouts are not valid for device access, [VkPipelineLayout](../../../../refpages/latest/refpages/source/VkPipelineLayout.html) objects cannot be created with such descriptor set layouts.
