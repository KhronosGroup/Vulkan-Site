...
To link to refpages for existing APIs or extensions, use the `xref:` macro
followed by the `refpages` attribute, the API name, and the `.html` suffix:

Example: [VkGraphicsPipelineCreateInfo](../../../../refpages/latest/refpages/source/VkGraphicsPipelineCreateInfo.html)

Example markup:

xref:refpages:refpages:source/VkGraphicsPipelineCreateInfo.adoc[VkGraphicsPipelineCreateInfo]

To link to an anchor in the specification other than an API or extension
which has its own refpage, use the `xref:` macro followed by the `docs`
attribute, the relative path of the file containing that anchor within the
spec repository, the `.html` suffix, and the anchor:

Example: [copies](../../../../spec/latest/chapters/copies.html#copies)

Example markup:

xref:spec:ROOT:chapters/copies.adoc#copies[copies]

If adding an image to a proposal document, it must be located in
`../images/proposals/`, and referred to with the `image:` macro
followed by the `images` attribute:

image::{images}/proposals/filename.svg[image options]

Only use the `link:` macro when linking to an external website (one not
found in the Vulkan Docs site). |
...
This section should identify any constraints on a solution that might not be apparent from the problem statement.
This might include things it must do from wider context (e.g. being usable!), things it absolutely is not addressing, or things that are addressed in very specific ways.

If any alternative solutions have been considered and dismissed, they should also be documented briefly here, with justifications.
...
This section is for anything that could be a useful addition moving forward, but may not be necessary to address the core problem.
