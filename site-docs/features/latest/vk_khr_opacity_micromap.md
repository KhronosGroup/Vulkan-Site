...
is ignored if the shader does not enable the `OpacityMicromapIdKHR` execution mode.
...
* 
`OpacityMicromapIdKHR` - this execution mode takes a specialization constant boolean value to determine
...
A built-in to enable encoding the `OpacityMicromapIdKHR` execution mode in SPIR-V is added in the
...
provide a shader that uses `SPV_KHR_opacity_micromap` if `OpacityMicromapIdKHR` is encoded.
