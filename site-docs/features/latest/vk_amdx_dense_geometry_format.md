...
For [vkGetAccelerationStructureBuildSizesKHR](../../../../refpages/latest/refpages/source/vkGetAccelerationStructureBuildSizesKHR.html) the application must supply both the total number of triangles and the expected size of the compressed data.  The latter is required so that implementations can use a 1:1 mapping between compressed blocks and acceleration structure nodes.
...
The `OPAQUE` flag is also supplied on a per-triangle basis.  The `VK_GEOMETRY_OPAQUE_BIT_KHR` flag on the [VkAccelerationStructureGeometryKHR](../../../../refpages/latest/refpages/source/VkAccelerationStructureGeometryKHR.html) structure has no effect for compressed geometry.

Multiple [VkAccelerationStructureGeometryKHR](../../../../refpages/latest/refpages/source/VkAccelerationStructureGeometryKHR.html) structures can be used to glue disconnected blobs of compressed data together into a larger acceleration structure, or for specifying different values for `VK_GEOMETRY_NO_DUPLICATE_ANY_HIT_INVOCATION_BIT_KHR` for different sets of DGF blocks.
...
To build acceleration structures using pre-compressed triangle data, the [VkAccelerationStructureGeometryKHR](../../../../refpages/latest/refpages/source/VkAccelerationStructureGeometryKHR.html) structure is extended using a new enum value in [VkGeometryTypeKHR](../../../../refpages/latest/refpages/source/VkGeometryTypeKHR.html):
...
When the `geometryType` member of [VkAccelerationStructureGeometryKHR](../../../../refpages/latest/refpages/source/VkAccelerationStructureGeometryKHR.html) is set to `VK_GEOMETRY_TYPE_DENSE_GEOMETRY_FORMAT_TRIANGLES_AMDX`, a `VkAccelerationStructureDenseGeometryFormatTrianglesDataAMDX` structure in its `pNext` chain describes pre-compressed triangle geometry:
...
Note that a host builds are not supported, and a [VkAccelerationStructureBuildRangeInfoKHR](../../../../refpages/latest/refpages/source/VkAccelerationStructureBuildRangeInfoKHR.html) structure is not used when building an acceleration structure with a geometry type of `VK_GEOMETRY_TYPE_DENSE_GEOMETRY_FORMAT_TRIANGLES_AMDX`.

To use an [Opacity Micromap](#spec:ROOT:chapters/VK_EXT_opacity_micromap/micromaps.adoc) with the compressed triangle data, a [VkAccelerationStructureTrianglesOpacityMicromapEXT](../../../../refpages/latest/refpages/source/VkAccelerationStructureTrianglesOpacityMicromapEXT.html) structure can be added to the `pNext` chain of `VkAccelerationStructureDenseGeometryFormatTrianglesDataAMDX`.
