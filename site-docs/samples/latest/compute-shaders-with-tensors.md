...
- **Component**: samples
- **Version**: latest
- **URL**: /samples/latest/samples/extensions/tensor_and_data_graph/compute_shaders_with_tensors/README.html
...
- [Accessing tensors from compute shaders](#_accessing_tensors_from_compute_shaders)
- [Accessing_tensors_from_compute_shaders](#_accessing_tensors_from_compute_shaders)
- [Binding tensors to compute shaders](#_binding_tensors_to_compute_shaders)
- [Binding_tensors_to_compute_shaders](#_binding_tensors_to_compute_shaders)
- [Tensor memory barriers](#_tensor_memory_barriers)
- [Tensor_memory_barriers](#_tensor_memory_barriers)
- [Conclusion](#_conclusion)

## Content

|  | The source for this sample can be found in the [Khronos Vulkan samples github repository](https://github.com/KhronosGroup/Vulkan-Samples/tree/main/samples/extensions/extensions/tensor_and_data_graph/compute_shaders_with_tensors). |
| --- | --- |

![Sample](../../../../_images/samples/extensions/tensor_and_data_graph/compute_shaders_with_tensors/images/sample.png)

This is the third sample in a series, which follows on from the [`graph_constants`](../graph_constants/README.html) sample. To best understand this sample, it would be helpful to have first looked at the [previous samples in the series](../README.html). This adds the use of compute shaders for generating the input to and processing the output of the data graph pipeline.
...
![Flow](../../../../_images/samples/extensions/tensor_and_data_graph/compute_shaders_with_tensors/images/flow.svg)
...
`tensorARM` which indicates that both are rank-4 tensors with float elements.
We also add the `writeonly` keyword to the output.

layout (set = 0, binding = 0) uniform tensorARM input_tensor;
layout (set = 0, binding = 1) writeonly uniform tensorARM output_tensor;
