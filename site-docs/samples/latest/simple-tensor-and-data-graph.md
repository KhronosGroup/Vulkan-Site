...
- **Component**: samples
- **Version**: latest
- **URL**: /samples/latest/samples/extensions/tensor_and_data_graph/simple_tensor_and_data_graph/README.html
...
- [Graph Pipelines](#_graph_pipelines)
- [Pipeline interface](#_pipeline_interface)
- [Pipeline](#_pipeline)
- [Pipeline session](#_pipeline_session)
- [Tensors](#_tensors)
- [Descriptor sets](#_descriptor_sets)
- [Command buffer](#_command_buffer)
- [Practical Use](#_practical_use)
- [Conclusion](#_conclusion)
- [Appendix A: SPIR-V programs for neural networks](#_appendix_a_spir_v_programs_for_neural_networks)
- [Appendix_A:_SPIR-V_programs_for_neural_networks](#_appendix_a_spir_v_programs_for_neural_networks)
- [Boilerplate](#_boilerplate)
- [Descriptor set interface](#_descriptor_set_interface)
- [Descriptor_set_interface](#_descriptor_set_interface)
- [Types and constants](#_types_and_constants)
- [Types_and_constants](#_types_and_constants)
- [Interface variables](#_interface_variables)
- [Graph definition](#_graph_definition)
...
This is the first in a series of samples which introduce the tensor and data graph extensions. Please see the parent [Tensor and Data Graph](../README.html) page for the full list of samples.

|  | While the model used in this example is expressed using [TOSA operations](https://www.mlplatform.org/tosa/), there is currently no defined way to query for TOSA operation support from the Vulkan API. Execution of this sample may fail on implementations of these extensions that do not include specific support for TOSA operations. |
| --- | --- |
...
![Sample](../../../../_images/samples/extensions/tensor_and_data_graph/simple_tensor_and_data_graph/images/flow.svg)

The image below shows how this sample will look when running:

![Sample](../../../../_images/samples/extensions/tensor_and_data_graph/simple_tensor_and_data_graph/images/sample.png)

![Sample](../../../../_images/samples/extensions/tensor_and_data_graph/simple_tensor_and_data_graph/images/network.svg)
...
![Sample](../../../../_images/samples/extensions/tensor_and_data_graph/simple_tensor_and_data_graph/images/pipeline_interface.svg)
...
std::set tensor_bindings = {0, 1};
std::vector layout_bindings;
...
std::vector dimensions = {1, 10, 10, 3};
...
* 
Aliased as a regular `VkImage` and used as a texture to be sampled from in another shader. See the "Tensor image aliasing" sample [README](../tensor_image_aliasing/README.html).
...
* 
Aliased as a regular `VkImage` and written to via a render target (or writable texture).  See the "Tensor image aliasing" sample [README](../tensor_image_aliasing/README.html).
...
The assembly code is in the [pooling.spvasm](../../../../shaders/tensor_and_data_graph/simple_tensor_and_data_graph/spirv/pooling.spvasm) file and is assembled into SPIR-V binary code using `spirv-as` as part of the [build system](../../../../bldsys/cmake/sample_helper.cmake). Note this is exactly the same process that would be used to compile compute or graphics shaders from SPIR-V assembly, although typically these would be compiled from GLSL or HLSL instead.

The SPIR-V assembly starts with the usual list of capabilities and extensions. Here, we declare that we will make use of the [Graph](https://github.khronos.org/SPIRV-Registry/extensions/ARM/SPV_ARM_graph.html) and [Tensors](https://github.khronos.org/SPIRV-Registry/extensions/ARM/SPV_ARM_tensors.html) SPIR-V extensions:
...
Next is an important line which imports [external instructions](https://registry.khronos.org/SPIR-V/specs/unified1/SPIRV.html#_extended_instruction_sets) for all of the neural network operations. This defines operations like convolution, pooling, matmul which come from the [TOSA specification](https://www.mlplatform.org/tosa/).

             %tosa = OpExtInstImport "TOSA.001000.1"

The next section declares the descriptor set and bindings that form the interface between the Vulkan API data graph pipeline and the SPIR-V module, and is what we used when defining our pipeline layout in the [Pipeline interface](#_pipeline_interface) section.
...
The `OpGraphEntryPointARM` instruction exports the graph (which will be defined in the lines below) to be visible to our C++ code under the entry point name `"main"`, which we used when creating our data graph [Pipeline](#_pipeline). It connects the interface variables declared above to the inputs and outputs of the data graph.

The lines between `OpGraphARM` and `OpGraphEndARM` define the body of our graph and is where the neural network operations are listed. Here we are using a single instruction imported from the TOSA SPIR-V [extended instruction set](#_boilerplate): AVG_POOL2D. That instruction requires a number of arguments describing the properties of the pooling operation (the pooling kernel size, striding, etc.).
