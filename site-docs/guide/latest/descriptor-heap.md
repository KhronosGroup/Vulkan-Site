...
- [VkDescriptorMappingSourceEXT bindingCount alias](#_vkdescriptormappingsourceext_bindingcount_alias)
- [VkDescriptorMappingSourceEXT_bindingCount_alias](#_vkdescriptormappingsourceext_bindingcount_alias)
...
- [Binding the heap](#_binding_the_heap)
- [Binding_the_heap](#_binding_the_heap)
- [Descriptor alignment](#_descriptor_alignment)
- [Indirect Buffer alignment](#_indirect_buffer_alignment)
- [Indirect_Buffer_alignment](#_indirect_buffer_alignment)
- [Null Descriptors](#_null_descriptors)
...
There is no descriptors arrays allowed (because there is no defined math how that would work).

The descriptor types are limited (images and samplers are not allowed)
...
A `Shader Record`` refers to an entry in the [Shader Binding Table (SBT)](https://docs.vulkan.org/spec/latest/chapters/raytracing.html#shader-binding-table)
...
"Untyped" is not the official spec term. There is a new [SPV_EXT_descriptor_heap](https://github.com/KhronosGroup/SPIRV-Guide/blob/main/chapters/descriptor_heap.md) extension that provides these heaps in SPIR-V, but to access them, you need to use SPIR-V untyped pointers. You could use `SPV_KHR_untyped_pointers` without `SPV_EXT_descriptor_heap` but it is not very uncommon.
...
![descriptor_heap_bind_alignment.svg](_images/descriptor_heap_bind_alignment.svg)

|  | The spec allows these alignments to be as high as 64k, but currently seems to at most
| --- | --- |
[64](https://vulkan.gpuinfo.org/displayextensionproperty.php?extensionname=VK_EXT_descriptor_heap&extensionproperty=resourceHeapAlignment&platform=all)
or
[32](https://vulkan.gpuinfo.org/displayextensionproperty.php?extensionname=VK_EXT_descriptor_heap&extensionproperty=samplerHeapAlignment&platform=all)
on most devices. |
...
![descriptor_heap_offset_alignment.svg](_images/descriptor_heap_offset_alignment.svg)

Second is when you do descriptor indexing that are non-packed (strided).

![descriptor_heap_stride_alignment.svg](_images/descriptor_heap_stride_alignment.svg)
...
![descriptor_heap_indirect_alignment.svg](_images/descriptor_heap_indirect_alignment.svg)
...
The [nullDescriptor](https://docs.vulkan.org/spec/latest/chapters/features.html#features-nullDescriptor) feature is a form of [robustness](robustness.html) that allows descriptors to be both null and valid. If you write a null descriptor the writes are discarded. If you read from a null descriptor it will return a defined value (mainly zero).
...
uint32_t combined_index = (image_index & 0xfffff) | ((sampler_index & 0xfff)
