...
- [Slang-specific Types](#_slang_specific_types)
...
- [Slang Interface Abstractions](#_slang_interface_abstractions)
- [Slang_Interface_Abstractions](#_slang_interface_abstractions)
- [Examples](#_examples)
- [Shader interface](#_shader_interface)
- [Slang Parameter Blocks](#_slang_parameter_blocks)
- [Slang_Parameter_Blocks](#_slang_parameter_blocks)
...
- [Misc](#_misc)
- [Slang Modules and Imports](#_slang_modules_and_imports)
- [Slang_Modules_and_Imports](#_slang_modules_and_imports)
- [Module Declaration](#_module_declaration)
- [Importing Modules](#_importing_modules)
- [Hierarchical Modules](#_hierarchical_modules)
- [Module Benefits](#_module_benefits)
...
While Vulkan itself consumes shaders in a binary format called [SPIR-V](what_is_spirv.html), shaders are usually written in a high level language. This section provides a mapping between shader functionality for the most common ones used with Vulkan: GLSL, HLSL, and Slang. This is mostly aimed at people wanting to migrate from one high level shader language to another. It’s meant as a starting point and not as a complete porting guide to one language from another.

|  | For more details on using HLSL with Vulkan, visit [this chapter](hlsl.html).
| --- | --- |

For more details on using Slang with Vulkan, visit [this chapter](slang.html). |
...
|  | Slang uses the same basic data types as HLSL (float*n*, int*n*, float*nxm*, etc.) but adds several advanced features like interfaces and full generic support. |
| --- | --- |
...
* 
[Slang data types](https://docs.shader-slang.org/en/latest/external/core-module-reference/types/index.html)
...
DXC supports [SPIR-V intrinsics](https://github.com/microsoft/DirectXShaderCompiler/wiki/GL_EXT_spirv_intrinsics-for-SPIR-V-code-gen) with the `GL_EXT_spirv_intrinsics` extension. This adds support for embedding arbitrary SPIR-V in the middle of GLSL for features not available in DirectX. For this new keywords are added to the `vk` namespace that map SPIR-V opcodes, incl. `vk::ext_extension`, `vk::ext_capability`, `vk::ext_builtin_input`, `vk::ext_execution_mode` and `vk::ext_instruction`.
...
|  | While GLSL makes heavy use of input and output variables built into the languages called "built-ins", there is no such concept in HLSL or Slang. HLSL and Slang instead use [semantics](https://learn.microsoft.com/en-us/windows/win32/direct3dhlsl/dx-graphics-hlsl-semantics), strings that are attached to inputs or outputs that contain information about the intended use of that variable. They are prefixed with `SV_`. For HLSL and Slang, input values are explicit arguments for the main entry point and the shader needs to explicitly return an output.
| --- | --- |
...
* 
[Slang Language Reference](https://github.com/shader-slang/spec)
