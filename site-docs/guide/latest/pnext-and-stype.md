...
The Vulkan API [provides two base structures](https://docs.vulkan.org/spec/latest/chapters/fundamentals.html#fundamentals-validusage-pNext), `VkBaseInStructure` and `VkBaseOutStructure`, to be used as a convenient way to iterate through a structure pointer chain.
...
void vkGetValue(VkA* pA) {
    ...
    VkBaseOutStructure* next = reinterpret_cast(pA->pNext);
    while (next != nullptr) {
        switch (next->sType) {

            case VK_STRUCTURE_TYPE_B:
                VkB* pB = reinterpret_cast(next);
