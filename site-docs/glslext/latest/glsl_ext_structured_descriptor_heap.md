# GLSL_EXT_structured_descriptor_heap

## Metadata

- **Component**: glslext
- **Version**: latest
- **URL**: /glslext/latest/glslext/ext/GLSL_EXT_structured_descriptor_heap.html
...
Overview
    ...
    There is also a new "buffer_type" qualifier for `uniform` and `buffer`
    block declarations, allowing their types to be embedded in the resource
    heap.
    ...
    layout(buffer_type, scalar) uniform MyUbo {
        uint data;
    };
    ...
    layout(buffer_type, scalar) buffer MyFirstSsbo {
        vec4 data;
    };

    layout(buffer_type, scalar) buffer MySecondSsbo {
        uint data;
    };

    layout (heap_offset = myPushData.buffersOffset) resourceheap BufferHeap {
        uint bufferCount;
        layout (descriptor_size = 128) MyFirstSsbo myFirstBuffer;
        buffer MySecondSsbo myStorageBuffers[];
    } bufferHeap;
    ...
    heap.
    Layout of data in a resource heap is always as if `scalar` layout were
    used for a buffer.
    The size of resource types in the heap is dependent on the client API,
    though can be overridden with the "descriptor_size" qualifier.
    ...
    // Offset and Stride decorations
                                  OpMemberDecorateIdEXT %materials_t 0 OffsetIdEXT %offset_0
    ...
    %texture_size               = OpConstantSizeOfEXT %uint_t %texture_t
    %buffer_size                = OpConstantSizeOfEXT %uint_t %uniform_buffer_t
    %vec4_size                  = OpConstant %uint_t 16
    %vec4_alignment             = OpConstant %uint_t 4  // Only type with alignment != size known to the shader
    %uint_size                  = OpConstant %uint_t 4
    ...
    %plain_old_data_align       = OpConstant %uint_t 4 // We know the largest alignment of POD statically
    ...
    %offset_1_padding           = OpSpecConstantOp OpUMod %uint_t %offset_1_unaligned %buffer_size
    ...
    %offset_2_padding           = OpSpecConstantOp OpUMod %uint_t %offset_2_unaligned %vec4_alignment
    ...
    %offset_3_padding           = OpSpecConstantOp OpUMod %uint_t %offset_3_unaligned %uint_size
    ...
    %materials_padding          = OpSpecConstantOp OpUMod %uint_t %materials_size_unaligned %materials_alignment
    %materials_size             = OpSpecConstantOp OpIAdd %uint_t %materials_size_unaligned %materials_padding
    ```
...
Modifications to the OpenGL Shading Language Specification, Version 4.60
    ...
    language features described in this extension:
      ...
      #extension GL_EXT_structured_descriptor_heap : 
...
(Variables and Types)
    ...
    Add to the end of section 4.3.9 (Interface Blocks)

    Uniform and shader storage blocks can be declared with the "buffer_type"
    layout qualifier, which alters their behavior to create an opaque buffer
    type that describes a block (as if the block were a structure type). The
    buffer type's name is the block name. There must be no instance name.
    Set and binding layouts as well as qualifiers like coherent and volatile
    don't apply to the buffer type, and can instead be applied to instances
    of the buffer type.

    The buffer type can be used to declare block members in the
    `resourceheap` interface, either directly or nested as part of a
    `struct` type.

    The field selector ( . ) operator is used to access members through a
    buffer type instance. This is the only operator allowed to operate on
    buffer types.
    ...
    `resourceheap` blocks may contain plain-old-data variables, buffer types,
    image*, texture*, and accelerationStructure* declarations, and buffer
    references.
    ...
    `samplerheap` blocks may contain plain-old-data variables, sampler*
    ...
    |----------------------------------------------------------------------------------|
    | buffer_type         |    X      |            |   X   |            | uniform      |
    |                     |           |            |       |            | /buffer      |
    |----------------------------------------------------------------------------------|
    | descriptor_size   = |           |            |       |      X     | resourceheap |
    |                     |           |            |       |            | /samplerheap |
    |----------------------------------------------------------------------------------|
    ...
    Add "buffer_type" to the layout-qualifier-id grammar rule.
    ...
    Add to the end of the section:
        ...
        otherwise specified by `descriptor_size`.
...
Revision History
    ...
    ----  -----------   --------   -------------------------------------------
     2    23-Apr-2026   thector    Add buffer types instead of inline blocks.
                                   Allow buffer references in sampler heap.
