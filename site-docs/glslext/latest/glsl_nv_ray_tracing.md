...
- **Component**: glslext
- **Version**: latest
- **URL**: /glslext/latest/glslext/nv/GLSL_NV_ray_tracing.html
...
(Built-in Variables)
    ...
    closest-hit, and miss shaders to specify the  and 
...
(Built-in Functions)
    ...
    specified by  and .
     ...
     is any combination of built-in constants as defined in
    Section 7.3 "Built-In Constants".

     is an 8 bit mask which specifies the instances to be intersected
    ...
    the different values of the compile-time constant  which correspond the
    ...
    The  parameter influences computation of indices into the
     to locate a miss shader during a trace query. See
    the Ray Tracing chapter of the vulkan specification for more information.
    ...
    any-hit shader,  is committed as the new gl_RayTmaxNV value of the
    current ray and  is committed as the new value for gl_HitKindNV.
    ...
    for details.
     ...
     is a compile-time constant used to select an shader defined structure
    ...
    compile-time constant  which corresponds to callableDataNV/callableDataInNV
