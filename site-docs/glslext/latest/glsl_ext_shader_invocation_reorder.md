...
- **Component**: glslext
- **Version**: latest
- **URL**: /glslext/latest/glslext/ext/GLSL_EXT_shader_invocation_reorder.html
...
Modifications to the OpenGL Shading Language Specification, Version 4.60
    ...
    language features described in this extension:
      ...
      #extension GL_EXT_shader_invocation_reorder  : 
...
(Built-in Functions)
    ...
    Populates 'hitobject' representing a hit without tracing a ray.
     ...
     is the ray query from which the intersection parameters
    are copied.

     is the record index of the hit shader of
     to be encoded in the hit object.

     is a compile-time constant to select
    ...
    different values of the compile-time constant  which correspond
    ...
    The attributes from the hitObjectAttributeEXT variable selected by
     are encoded and stored in the hit object. These stored
    ...
    hitObjectExecuteShaderEXT, as described in the specification for that
    function. The type and structure of attributes stored must match the
    type expected when retrieved or used in closest-hit shaders. If the
    committed intersection in ray query is for triangle geometry, the value
    is ignored and taken directly from ray query.

    Syntax:

        void hitObjectRecordFromQueryEXT(hitObjectEXT hitobject,
                    rayQueryEXT rayQuery, uint sbtRecordIndex
                    int attributeLocation, uint hitKind)
    ...
    is ignored and taken directly from ray query.
     ...
     is the integer hit kind to be encoded in the hit object.
    If the committed intersection in ray query is for triangle geometry,
    this value will be ignored and taken directly from ray query.
    ...
    could have hit some geometry if traced.
     ...
     refers to the record index of the miss shader of
    the  for which this hit object is created.
    ...
    Refer to 'traceRayMotionNV' for defintion of 
    ...
    selected by the  parameter when the attributes were stored.
    ...
    Returns 'true' if  encodes a empty hit object, 'false' otherwise.

    Syntax:

        bool hitObjectIsMissEXT(hitObjectEXT hitObject)

    Returns 'true' if  encodes a miss, 'false' otherwise.

    Syntax:

        bool hitObjectIsHitEXT(hitObjectEXT hitObject)

    Returns 'true' if  encodes a hit, 'false' otherwise.

    Syntax:

        float hitObjectGetRayTMinEXT(hitObjectEXT hitObject)

    Returns the parametric  value encoded in the hit object.
    ...
    Returns the  value encoded in the hit object.
    ...
    The type of the hitObjectAttributeEXT variable selected using 
    must match the type that was stored. The behavior is undefined if attributes were not
    stored in the hit object or if the types referenced by the  do not match.
    ...
    Reorder threads based on user provided . Similar hint values
    indicate similarity of subsequent work done after this call. Behavior
    is implementation defined.

     determines desired ordering of threads relative to others.

     indicates number of least significant bits an implementation
    should take into account from  in determining ordering.

    Value of  should be . The exact properties
    ...
    Reorder threads based on provided  supplemented by additional
    information based on user provided . The exact properties from
