...
- **Component**: glslext
- **Version**: latest
- **URL**: /glslext/latest/glslext/nv/GL_NV_shader_subgroup_partitioned.html
...
Overview
    ...
    All subgroupPartitionedNV, subgroupPartitionedInclusiveNV
    and subgroupPartitionedExclusiveNV functions map to
    OpGroupNonUniform:
    ...
    When using GroupOperationPartitioned*, the  parameter is the last
    operand to OpGroupNonUniform, similar to ClusterSize.
...
Modifications to the OpenGL Shading Language Specification, Version 4.50
    ...
    language features described in this extension:
      ...
      #extension GL_NV_shader_subgroup_partitioned       : 
...
(Basics)
    ...
    (Add to the end of the Subgroup section)
        ...
        There are three classes of subgroup built-in functions that take a
         parameter: subgroupPartitionedInclusiveNV(),
        subgroupPartitionedExclusiveNV(), and subgroupPartitionedNV(),
        where  is one of: Add, Mul, Min, Max, And, Or, Xor

        The  parameter to these functions must form a valid partition
        of the active invocations in the subgroup. The values of  are
        a valid partition if:

          * for each active invocation *, the bit corresponding to  is
            set in 's value of , and
          * for any two active invocations  and , if the bit
            corresponding to invocation  is set in invocation 's value
            of , then invocation 's value of  must equal
            invocation 's value of , and
          * bits not corresponding to any invocation in the subgroup are
            ignored.

        If two active invocations  and  have the same value of ,
        they are said to be "in the same subset of the partition".

        subgroupPartitionedInclusiveNV(),
        subgroupPartitionedExclusiveNV(), and
        subgroupPartitionedNV() perform an inclusive scan, exclusive scan,
        ...
        subgroupInclusive() and subgroupExclusive() functions, the
        ...
        of , and each of our 8 invocations is active
        within the subgroup.
        ...
        subgroupPartitionedAddNV() operation will produce the float 
        [121.0, 15.5, 121.0, 15.5, 121.0, 15.5, 121.0, 15.5].

        If the  parameter to any partitioned subgroup operation is
...
(Built-in Functions)
    ...
    provided s in the invocation's subset of the partition.  The method that is used to perform the operation on
    ...
    invocation-provided s in the invocation's subset of the partition.  The method that is used to perform the
    ...
    The function subgroupPartitionedMinNV() returns the minimum  of all active
    invocation-provided s in the invocation's subset of the partition.
    ...
    The function subgroupPartitionedMaxNV() returns the maximum  of all active
    invocation-provided s in the invocation's subset of the partition.
    ...
    AND of all active invocation provided s in the invocation's subset of the partition.  For genBType, the function
    subgroupPartitionedAndNV() returns the logical AND of all active invocation provided
    s in the invocation's subset of the partition.
    ...
    OR of all active invocation provided s in the invocation's subset of the partition.  For genBType, the function
    subgroupPartitionedOrNV() returns the logical inclusive OR of all active invocation
    provided s in the invocation's subset of the partition.
    ...
    XOR of all active invocation provided s in the invocation's subset of the partition.  For genBType, the function
    ...
    that is the summation of all active invocation-provided s in the invocation's subset of the partition.  The
    ...
    that is the multiplication of all active invocation-provided s in the invocation's subset of the partition.
    ...
    that is the minimum  of all active invocation-provided s in the invocation's subset of the partition.
    ...
    that is the maximum  of all active invocation-provided s in the invocation's subset of the partition.
    ...
    invocation-provided s in the invocation's subset of the partition.  For genBType, the function
    subgroupPartitionedInclusiveAndNV() returns an inclusive scan operation that is the
    logical AND of all active invocation-provided s in the invocation's subset of the partition.
    ...
    logical inclusive OR of all active invocation-provided s in the invocation's subset of the partition.
    ...
    logical exclusive OR of all active invocation-provided s in the invocation's subset of the partition.
   ...
   Syntax:
    ...
    that is the summation of all active invocation-provided s in the invocation's subset of the partition.
    The method used to perform the operation on each active invocation's 
    is implementation defined.
    ...
    subset of the partition have the same value of . For any two
    invocations in different subsets of the partition, either their values of
     must not be equal or one must be a floating point NaN.
