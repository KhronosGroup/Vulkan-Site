...
Memory locations are implicitly allocated for SPIR-V variables by the
implementation.
Buffers and [images](../chapters/images.html#images) are explicitly mapped to memory locations
allocated in [VkDeviceMemory](../chapters/memory.html#VkDeviceMemory) objects.
...
`OpMemoryBarrier`, and `OpMemoryNamedBarrier`.
...
`OpControlBarrier`,
`OpControlBarrierArriveEXT`, `OpControlBarrierWaitEXT`,
`OpMemoryBarrier`, and `OpMemoryNamedBarrier` are *memory barrier*
instructions in SPIR-V.
`OpControlBarrierArriveEXT` is a *control barrier arrival* and
`OpControlBarrierWaitEXT` is a *control barrier wait*.
...
If A is a release barrier, B is an acquire barrier, C is a control barrier
arrival, and D is a control barrier wait (where A **can** equal C, and B **can**
equal D), then A synchronizes-with B if all of the following are true:

* 
A is program-ordered before (or equals) C

* 
D is program-ordered before (or equals) B

* 
C and D have the same execution scope
