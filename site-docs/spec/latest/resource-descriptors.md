...
or
[descriptor sets](descriptorsets.html#descriptors-sets).
...
`opImageBlockMatchGatherSAD`, and `opImageBlockMatchGatherSSD`
operations, only the single-component formats with the
[VK_FORMAT_FEATURE_2_BLOCK_MATCHING_BIT_QCOM](formats.html#VkFormatFeatureFlagBits2KHR) flag set are supported.

For `OpImageBlockMatchSSDQCOM` and `OpImageBlockMatchSADQCOM`
operations, additional formats for target view and reference view are
specified with the
[](formats.html#formats-properties)[VK_FORMAT_FEATURE_2_BLOCK_MATCHING_SXD_BIT_QCOM](formats.html#VkFormatFeatureFlagBits2KHR)
format feature flag.

|  | Applications **should** use
| --- | --- |
[VK_FORMAT_FEATURE_2_BLOCK_MATCHING_SXD_BIT_QCOM](formats.html#VkFormatFeatureFlagBits2KHR) to determine the full
format support with `OpImageBlockMatchSSDQCOM` and
`OpImageBlockMatchSADQCOM`.
Applications **can** also use
[VK_FORMAT_FEATURE_2_BLOCK_MATCHING_BIT_QCOM](formats.html#VkFormatFeatureFlagBits2KHR), but it **may** specify a
reduced set of formats that are also compatible with the
