...
* 
[](#VUID-VkMemoryAllocateInfo-pNext-12500) VUID-VkMemoryAllocateInfo-pNext-12500

  If the parameters define an import operation, the external handle is an
  Android hardware buffer, the `pNext` chain includes a
  [VkMemoryDedicatedAllocateInfo](#VkMemoryDedicatedAllocateInfo) with `image` that is not
  [VK_NULL_HANDLE](../appendices/boilerplate.html#VK_NULL_HANDLE), the `image` was created with a
  [VkExternalFormatANDROID](resources.html#VkExternalFormatANDROID) with a non-zero `externalFormat`, and
  the
  [VkAndroidHardwareBufferFormatProperties2ANDROID](#VkAndroidHardwareBufferFormatProperties2ANDROID)::`formatFeatures`
  returned by [vkGetAndroidHardwareBufferPropertiesANDROID](#vkGetAndroidHardwareBufferPropertiesANDROID) for the
  Android hardware buffer does not contain
  [VK_FORMAT_FEATURE_2_HOST_IMAGE_TRANSFER_BIT](formats.html#VkFormatFeatureFlagBits2KHR), the `image` **must**
  not have been created with a `usage` containing
