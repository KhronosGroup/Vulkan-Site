...
Each texel block in an image maps to a distinct set of
[memory locations](../appendices/memorymodel.html#memory-model-memory-location) within the memory they are
bound to.
Unless otherwise noted by the specification, the mapping from texels to
...
If the coordinates are [out of bounds](shaders.html#shaders-execution-memory-access-bounds), the write is discarded.
...
[VkImageSubresourceRange](resources.html#VkImageSubresourceRange)::`levelCount` specified for the image
view used.
