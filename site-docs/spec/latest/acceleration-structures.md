...
the
[VK_BUILD_ACCELERATION_STRUCTURE_ALLOW_OPACITY_MICROMAP_DATA_UPDATE_BIT_EXT](resources.html#VkBuildAccelerationStructureFlagBitsNV)
flag **must** provide a micromap deserialized using the serialized output of
the micromap that was originally referenced by the serialized acceleration
structure.
...
from the original micromap’s serialized data if the original acceleration
structure was deserialized prior.
