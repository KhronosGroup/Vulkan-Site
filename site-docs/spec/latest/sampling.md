...
- [Instruction/Format/Sampler/Image View Validation](#textures-operation-validation)
- [Instruction/Format/Sampler/Image_View_Validation](#textures-operation-validation)
...
`OpImage*Gather`,
`OpImageGatherQCOM`,
    and `OpImageSparse*Gather` read neighboring texels and
    [return a single component](#textures-gather) of each.
...
[Instruction/Format/Sampler/Image     validation](#textures-operation-validation)
...
with the image view’s or buffer view’s `format`.
...
If the image view’s `viewType` is [VK_IMAGE_VIEW_TYPE_1D_ARRAY](resources.html#VkImageViewType)
     or [VK_IMAGE_VIEW_TYPE_2D_ARRAY](resources.html#VkImageViewType),
and the image was not created with [VK_IMAGE_CREATE_ALIAS_SINGLE_LAYER_DESCRIPTOR_BIT_KHR](resources.html#VkImageCreateFlagBits),
     then the instruction **must** have `Arrayed` = 1.

* 
If the image view’s `viewType` is [VK_IMAGE_VIEW_TYPE_1D](resources.html#VkImageViewType) or
     [VK_IMAGE_VIEW_TYPE_2D](resources.html#VkImageViewType),
and the image was not created with [VK_IMAGE_CREATE_ALIAS_SINGLE_LAYER_DESCRIPTOR_BIT_KHR](resources.html#VkImageCreateFlagBits),
     the instruction **must** have `Arrayed` = 0.

* 
If the image view’s `viewType` is [VK_IMAGE_VIEW_TYPE_1D_ARRAY](resources.html#VkImageViewType)
or [VK_IMAGE_VIEW_TYPE_2D_ARRAY](resources.html#VkImageViewType) and `Arrayed` = 0, the image
view **must** have been created with a single layer.

* 
If the image view’s `viewType` is
[VK_IMAGE_VIEW_TYPE_CUBE_ARRAY](resources.html#VkImageViewType) then the instruction **must** have
`Arrayed` = 1.

* 
If the image view’s `viewType` is [VK_IMAGE_VIEW_TYPE_CUBE](resources.html#VkImageViewType)
then the instruction **must** have `Arrayed` = 0.
...
|  | When accessing storage images or texel buffers, there are many valid usage
| --- | --- |
statements that prevent the `OpTypeImage` and [VkFormat](formats.html#VkFormat) from being
different with respect to things such as numeric type, number of components,
etc.
Texel output validation covers cases such as using `R32ui` with
[VK_FORMAT_R8_UINT](formats.html#VkFormat) that for historical reasons are not considered
invalid usage, but still result in **undefined** values.
Validation layers will provide a warning in these **undefined** value cases. |
...
identify the four selected texels for `OpImage*Gather`
, and `OpImageGatherQCOM` with modes `GatherH2QCOM` and
`GatherV2QCOM`
.

If an `OpImage*Gather` operation does not use the `ConstOffsets` image
operand then the four texels form the 2 × 2 rectangle used for texture
filtering:
...
If an `OpImage*Gather` operation uses the `ConstOffsets` image operand
then the offsets allow a custom filter to be defined:

  

  

If an `OpImageGatherQCOM` operation has a `Mode` equal to
`GatherH2QCOM` or `GatherV2QCOM`, then it uses a predefined set of
`ConstOffsets`.

The offsets used for `GatherH2QCOM` are:

  

  

The offsets used for `GatherV2QCOM` are:

  

  

The rules for the [VK_FILTER_NEAREST](samplers.html#VkFilter) minification filter are applied to
identify the origin texel \tau_{{ij}} for `OpImageGatherQCOM` with
modes equal to `Gather4x1QCOM` and `GatherDQCOM`.
The filter used for these modes is defined as:

  

  

The offsets used for `Gather4x1QCOM` are:

  

  

The offsets used for `GatherDQCOM` are:
...
Block matching operations **must** be specified with identical samplers to use
with the *target* and *reference* images, which is defined as the *input*
sampler.

The *input* sampler **must** be created with:
  * For `opImageBlockMatchSAD` and `opImageBlockMatchSSD`,
    `addressModeU` and `addressModeV` equal to
    [VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE](samplers.html#VkSamplerAddressMode), or
  ...
  * For `opImageBlockMatchWindowSAD`, `opImageBlockMatchWindowSSD`,
    `opImageBlockMatchGatherSAD`, or `opImageBlockMatchGatherSSD`
    instructions, `addressModeU` and `addressModeV` equal to
    [VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_BORDER](samplers.html#VkSamplerAddressMode) with
    [VK_BORDER_COLOR_FLOAT_TRANSPARENT_BLACK](samplers.html#VkBorderColor)
    , or if the
    [`blockMatchExtendedClampToEdge`](features.html#features-blockMatchExtendedClampToEdge)
    feature is enabled, equal to [VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE](samplers.html#VkSamplerAddressMode)
  * For `opImageBlockMatchWindowSAD`, `opImageBlockMatchWindowSSD`,
    `opImageBlockMatchGatherSAD`, or `opImageBlockMatchGatherSSD`
    instructions, with either `addressModeU` and `addressModeV` equal
    to [VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE](samplers.html#VkSamplerAddressMode), `addressModeU` and
    `addressModeV` equal to [VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE](samplers.html#VkSamplerAddressMode)
  * `unnormalizedCoordinates` equal to [VK_TRUE](fundamentals.html#VK_TRUE)
  * `components` equal to [VK_COMPONENT_SWIZZLE_IDENTITY](resources.html#VkComponentSwizzle)
  * `minFilter` equal to [VK_FILTER_NEAREST](samplers.html#VkFilter)
  * `mipmapMode` equal to [VK_SAMPLER_MIPMAP_MODE_NEAREST](samplers.html#VkSamplerMipmapMode)
  * `minLod` and `maxLod` equal to `0`
  * `anisotropyEnable` equal to [VK_FALSE](fundamentals.html#VK_FALSE)
  * `compareEnable` equal to [VK_FALSE](fundamentals.html#VK_FALSE)
  * For `opImageBlockMatchWindowSAD` and `opImageBlockMatchWindowSSD`
    instructions, [VkSamplerBlockMatchWindowCreateInfoQCOM](samplers.html#VkSamplerBlockMatchWindowCreateInfoQCOM) **must** be
    included in the `pNext` chain.

Other sampler states are ignored.

|  | *Reference* image does not support wrap modes and the coordinates **must**
| --- | --- |
remain inside the image.
Application **must** specify wrap modes in the sampler object because it **must**
be identical with the *target* image’s sampler object, which does support
wrap modes. |

The *target* and *reference* images **must** be created with:
  * `imageType` equal to [VK_IMAGE_TYPE_2D](resources.html#VkImageType)
  * `mipLevels` equal to `1`
  * `arrayLayers` equal to `1`
  * `samples` equal to [VK_SAMPLE_COUNT_1_BIT](limits.html#VkSampleCountFlagBits)
  * `format` equal to a compatible [block    matching format](descriptors.html#descriptors-blockmatch)
  * `usage` equal to `VK_IMAGE_USAGE_SAMPLE_BLOCK_MATCH_BIT_QCOM`
...
If the
[`blockMatchExtendedClampToEdge`](features.html#features-blockMatchExtendedClampToEdge)
feature is enabled, and the *input* sampler was created with
`addressModeU` and `addressModeV` equal to
[VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE](samplers.html#VkSamplerAddressMode), then any reference texel
coordinate    in the set that fails
[integer texel coordinate validation](#textures-integer-coordinate-validation) is an invalid texel and [border replacement](#textures-border-replacement) is performed.
Otherwise, each
reference texel coordinate    in the set **must** not fail
integer texel coordinate validation.
To avoid **undefined** behavior, application shader **must** guarantee that the
...
`OpImageGatherQCOM`,
    and `OpImage*Gather` instructions.

* 
Texel Gathering: Performed by `OpImage*Gather`
and `OpImageGatherQCOM`
    instructions.
...
The return value is the vector (dl - levelbase, λ').
