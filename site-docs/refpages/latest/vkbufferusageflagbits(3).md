...
* 
[VK_BUFFER_USAGE_MICROMAP_BUILD_INPUT_READ_ONLY_BIT_EXT](#) specifies
that when building a [VkMicromapEXT](VkMicromapEXT.html), the buffer **can** be used as a
[read-only micromap build input](../../../../spec/latest/chapters/VK_KHR_opacity_micromap/micromaps.html#micromap-building), which includes the
`data` and `triangleArray` parameters

* 
[VK_BUFFER_USAGE_MICROMAP_STORAGE_BIT_EXT](#) specifies that the buffer
**can** be used to [create ](../../../../spec/latest/chapters/resources.html#resources-micromaps)[VkMicromapEXT](VkMicromapEXT.html)    objects.
