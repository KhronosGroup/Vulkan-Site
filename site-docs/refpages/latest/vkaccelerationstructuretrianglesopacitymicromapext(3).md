...
[VkOpacityMicromapSpecialIndexEXT](VkOpacityMicromapSpecialIndexKHR.html) then that triangle behaves as
...
If `micromap` is not [VK_NULL_HANDLE](VK_NULL_HANDLE.html), the `type` used to
construct `micromap` is [VK_MICROMAP_TYPE_OPACITY_MICROMAP_EXT](VkMicromapTypeEXT.html),
...
If `micromap` is not [VK_NULL_HANDLE](VK_NULL_HANDLE.html), the `type` used to
build `micromap` is [VK_MICROMAP_TYPE_OPACITY_MICROMAP_EXT](VkMicromapTypeEXT.html), for
each member of `pUsageCounts` or `ppUsageCounts` with
`format` equal to [VK_OPACITY_MICROMAP_FORMAT_2_STATE_EXT](VkOpacityMicromapFormatKHR.html),
...
build `micromap` is [VK_MICROMAP_TYPE_OPACITY_MICROMAP_EXT](VkMicromapTypeEXT.html), for
each member of `pUsageCounts` or `ppUsageCounts` with
`format` equal to [VK_OPACITY_MICROMAP_FORMAT_4_STATE_EXT](VkOpacityMicromapFormatKHR.html),
`subdivisionLevel` **must** be less than or equal to
...
[VK_MICROMAP_TYPE_DISPLACEMENT_MICROMAP_NV](VkMicromapTypeEXT.html), the `format` member
of each element of `pUsageCounts` or `ppUsageCounts` **must** be a
valid value from [VkDisplacementMicromapFormatNV](VkDisplacementMicromapFormatNV.html)
...
If `micromap` is not [VK_NULL_HANDLE](VK_NULL_HANDLE.html), the `type` used to
construct `micromap` is
[VK_MICROMAP_TYPE_DISPLACEMENT_MICROMAP_NV](VkMicromapTypeEXT.html), for each member of
