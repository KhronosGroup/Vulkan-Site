...
- **Component**: refpages
- **Version**: latest
- **URL**: /refpages/latest/refpages/source/VK_KHR_swapchain.html
...
`[VK_KHR_surface](VK_KHR_surface.html)` extension.
It introduces [VkSwapchainKHR](VkSwapchainKHR.html) objects, which provide the ability to
...
* 
[VkSwapchainCreateFlagBitsKHR](VkSwapchainCreateFlagBitsKHR.html)
...
* 
[VkSwapchainCreateFlagsKHR](VkSwapchainCreateFlagsKHR.html)
...
7) Does [vkAcquireNextImageKHR](vkAcquireNextImageKHR.html) block if no images are available?
...
8) Can multiple presents be queued using one [vkQueuePresentKHR](vkQueuePresentKHR.html) call?

**RESOLVED**: Yes.
[VkPresentInfoKHR](VkPresentInfoKHR.html) contains a list of swapchains and corresponding image
indices that will be presented.
When supported, all presentations queued with a single
[vkQueuePresentKHR](vkQueuePresentKHR.html) call will be applied atomically as one operation.
...
These can be returned by both [vkAcquireNextImageKHR](vkAcquireNextImageKHR.html) and
[vkQueuePresentKHR](vkQueuePresentKHR.html).

10) Does the [vkAcquireNextImageKHR](vkAcquireNextImageKHR.html) command return a semaphore to the
...
received from [vkAcquireNextImageKHR](vkAcquireNextImageKHR.html).
...
images when the call to [vkAcquireNextImageKHR](vkAcquireNextImageKHR.html) was made.
...
[VkSurfaceCapabilitiesKHR](VkSurfaceCapabilitiesKHR.html)::`maxImageCount` to indicate there are no
...
[VkSurfaceCapabilitiesKHR](VkSurfaceCapabilitiesKHR.html)::`currentExtent` to indicate the size of
...
[VkSurfaceCapabilitiesKHR](VkSurfaceCapabilitiesKHR.html)::`maxImageExtent` to indicate there is no
...
[VkSurfaceCapabilitiesKHR](VkSurfaceCapabilitiesKHR.html).
...
23) What is the purpose of specifying a [VkColorSpaceKHR](VkColorSpaceKHR.html) along with
...
A [VkSurfaceKHR](VkSurfaceKHR.html) owns the binding of the native window to the Vulkan
driver.
...
**RESOLVED**: For most platforms, the [VkSurfaceKHR](VkSurfaceKHR.html) structure is a simple
...
[VkSurfaceKHR](VkSurfaceKHR.html) objects that may be created for the same native window if
such a requirement makes sense on a particular platform, but a global
requirement is only sensible at the swapchain level.

|  | The example code for the `[VK_KHR_surface](VK_KHR_surface.html)` and `VK_KHR_swapchain`
| --- | --- |
...
For more information, see the [Vulkan Specification](../../../../spec/latest/appendices/extensions.html#VK_KHR_swapchain).
