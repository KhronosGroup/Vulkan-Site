...
Any [VkDeviceOrHostAddressKHR](VkDeviceOrHostAddressKHR.html), [VkDeviceOrHostAddressConstKHR](VkDeviceOrHostAddressConstKHR.html), or
...
For each element of either `pGeometries` or `ppGeometries` at a
given index, with a `geometryType` member equal to
[VK_GEOMETRY_TYPE_MICROMAP_KHR](VkGeometryTypeKHR.html), the sum of usage information in
...
[VkAccelerationStructureTrianglesOpacityMicromapKHR](VkAccelerationStructureTrianglesOpacityMicromapKHR.html) the
corresponding member of `pBuildInfo` also contains
[VkAccelerationStructureTrianglesOpacityMicromapKHR](VkAccelerationStructureTrianglesOpacityMicromapKHR.html) and with an
equivalent `micromap`.
...
[VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR](VkAccelerationStructureBuildTypeKHR.html)
