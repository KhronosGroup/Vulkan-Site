...
A [VkDataGraphPipelineSessionNeuralStatisticsCreateInfoARM](VkDataGraphPipelineSessionNeuralStatisticsCreateInfoARM.html)
structure **may** only be included in the `pNext` chain if and only if
a [VkDataGraphPipelineNeuralStatisticsCreateInfoARM](VkDataGraphPipelineNeuralStatisticsCreateInfoARM.html) structure whose
`allowNeuralStatistics` member was [VK_TRUE](VK_TRUE.html) was included in the
`pNext` chain of the [VkDataGraphPipelineCreateInfoARM](VkDataGraphPipelineCreateInfoARM.html)
...
If `flags` contains
[VK_DATA_GRAPH_PIPELINE_SESSION_CREATE_PROTECTED_BIT_ARM](VkDataGraphPipelineSessionCreateFlagBitsARM.html) then a
[VkDataGraphPipelineSessionNeuralStatisticsCreateInfoARM](VkDataGraphPipelineSessionNeuralStatisticsCreateInfoARM.html) structure
...
[](#VUID-VkDataGraphPipelineSessionCreateInfoARM-pNext-pNext) VUID-VkDataGraphPipelineSessionCreateInfoARM-pNext-pNext
 ...
 `pNext` **must** be `NULL` or a pointer to a valid instance of [VkDataGraphPipelineSessionNeuralStatisticsCreateInfoARM](VkDataGraphPipelineSessionNeuralStatisticsCreateInfoARM.html)
...
[](#VUID-VkDataGraphPipelineSessionCreateInfoARM-flags-parameter) VUID-VkDataGraphPipelineSessionCreateInfoARM-flags-parameter
 ...
 If `flags` is not `0`, `flags` **must** be a valid combination of [VkDataGraphPipelineSessionCreateFlagBitsARM](VkDataGraphPipelineSessionCreateFlagBitsARM.html) values
