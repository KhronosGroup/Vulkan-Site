...
When determining [pipeline layout compatibility](../../../../spec/latest/chapters/descriptorsets.html#descriptors-compatibility), compatibility is determined as if a the shader was created
...
[VkPipelineLayoutCreateInfo](VkPipelineLayoutCreateInfo.html)::`flags` set to 0.
If `flags` includes [VK_SHADER_CREATE_DESCRIPTOR_HEAP_BIT_EXT](VkShaderCreateFlagBitsEXT.html), the
...
[`maxPushDataSize`](../../../../spec/latest/chapters/limits.html#limits-maxPushDataSize) minus the
...
[VK_DESCRIPTOR_MAPPING_SOURCE_SHADER_RECORD_ADDRESS_EXT](VkDescriptorMappingSourceEXT.html), and there
...
[VK_DESCRIPTOR_MAPPING_SOURCE_PUSH_ADDRESS_EXT](VkDescriptorMappingSourceEXT.html), or
[VK_DESCRIPTOR_MAPPING_SOURCE_INDIRECT_ADDRESS_EXT](VkDescriptorMappingSourceEXT.html) the mapped
...
`setLayoutCount` valid [VkDescriptorSetLayout](VkDescriptorSetLayout.html) handles
...
`flags` includes [VK_SHADER_CREATE_INDEPENDENT_SETS_BIT_KHR](VkShaderCreateFlagBitsEXT.html),
then `pSetLayouts` must be a valid pointer to an array of
`setLayoutCount` [VkDescriptorSetLayout](VkDescriptorSetLayout.html) or `VK_NULL_HANDLE`
handles

* 
[](#VUID-VkShaderCreateInfoEXT-stage-12428) VUID-VkShaderCreateInfoEXT-stage-12428

If `stage` is [VK_SHADER_STAGE_COMPUTE_BIT](VkShaderStageFlagBits.html), then `flags`
**must** not include [VK_SHADER_CREATE_INDEPENDENT_SETS_BIT_KHR](VkShaderCreateFlagBitsEXT.html)
...
it **must** not include [VK_SHADER_CREATE_INDEPENDENT_SETS_BIT_KHR](VkShaderCreateFlagBitsEXT.html)
...
`stage` **must** not contain [VK_SHADER_STAGE_RAYGEN_BIT_KHR](VkShaderStageFlagBits.html),
...
[](#VUID-VkShaderCreateInfoEXT-flags-parameter) VUID-VkShaderCreateInfoEXT-flags-parameter
 ...
 If `flags` is not `0`, `flags` **must** be a valid combination of [VkShaderCreateFlagBitsEXT](VkShaderCreateFlagBitsEXT.html) values
...
[](#VUID-VkShaderCreateInfoEXT-nextStage-parameter) VUID-VkShaderCreateInfoEXT-nextStage-parameter
 ...
 If `nextStage` is not `0`, `nextStage` **must** be a valid combination of [VkShaderStageFlagBits](VkShaderStageFlagBits.html) values
