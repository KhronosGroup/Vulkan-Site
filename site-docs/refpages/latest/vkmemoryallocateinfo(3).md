...
[](#VUID-VkMemoryAllocateInfo-pNext-12500) VUID-VkMemoryAllocateInfo-pNext-12500
  ...
  [VkMemoryDedicatedAllocateInfo](VkMemoryDedicatedAllocateInfo.html) with `image` that is not
  [VK_NULL_HANDLE](VK_NULL_HANDLE.html), the `image` was created with a
  [VkExternalFormatANDROID](VkExternalFormatANDROID.html) with a non-zero `externalFormat`, and
  the
  [VkAndroidHardwareBufferFormatProperties2ANDROID](VkAndroidHardwareBufferFormatProperties2ANDROID.html)::`formatFeatures`
  returned by [vkGetAndroidHardwareBufferPropertiesANDROID](vkGetAndroidHardwareBufferPropertiesANDROID.html) for the
  Android hardware buffer does not contain
  [VK_FORMAT_FEATURE_2_HOST_IMAGE_TRANSFER_BIT](VkFormatFeatureFlagBits2.html), the `image` **must**
