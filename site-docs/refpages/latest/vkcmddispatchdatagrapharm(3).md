...
[VkDataGraphPipelineOpticalFlowDispatchInfoARM](VkDataGraphPipelineOpticalFlowDispatchInfoARM.html) structure, then the
`flags` member of that latter structure **must** not contain
[VK_DATA_GRAPH_OPTICAL_FLOW_EXECUTE_INPUT_UNCHANGED_BIT_ARM](VkDataGraphOpticalFlowExecuteFlagBitsARM.html),
[VK_DATA_GRAPH_OPTICAL_FLOW_EXECUTE_REFERENCE_UNCHANGED_BIT_ARM](VkDataGraphOpticalFlowExecuteFlagBitsARM.html),
[VK_DATA_GRAPH_OPTICAL_FLOW_EXECUTE_INPUT_IS_PREVIOUS_REFERENCE_BIT_ARM](VkDataGraphOpticalFlowExecuteFlagBitsARM.html),
or
[VK_DATA_GRAPH_OPTICAL_FLOW_EXECUTE_REFERENCE_IS_PREVIOUS_INPUT_BIT_ARM](VkDataGraphOpticalFlowExecuteFlagBitsARM.html)
...
If the [VkPipeline](VkPipeline.html) bound to the
[VK_PIPELINE_BIND_POINT_DATA_GRAPH_ARM](VkPipelineBindPoint.html) pipeline bind point was not
created with a [VkDataGraphPipelineSingleNodeCreateInfoARM](VkDataGraphPipelineSingleNodeCreateInfoARM.html)
structure whose `nodeType` member is
[VK_DATA_GRAPH_PIPELINE_NODE_TYPE_OPTICAL_FLOW_ARM](VkDataGraphPipelineNodeTypeARM.html) included in the
`pNext` chain of [VkDataGraphPipelineCreateInfoARM](VkDataGraphPipelineCreateInfoARM.html), then a
[VkDataGraphPipelineOpticalFlowDispatchInfoARM](VkDataGraphPipelineOpticalFlowDispatchInfoARM.html) structure **must** not
...
If the [VkPipeline](VkPipeline.html) bound to the
[VK_PIPELINE_BIND_POINT_DATA_GRAPH_ARM](VkPipelineBindPoint.html) pipeline bind point was
created with a [VkDataGraphPipelineSingleNodeCreateInfoARM](VkDataGraphPipelineSingleNodeCreateInfoARM.html)
structure whose `nodeType` member is
[VK_DATA_GRAPH_PIPELINE_NODE_TYPE_OPTICAL_FLOW_ARM](VkDataGraphPipelineNodeTypeARM.html) included in the
`pNext` chain of [VkDataGraphPipelineCreateInfoARM](VkDataGraphPipelineCreateInfoARM.html), then all
image subresources that are accessed via a connection point in
[VkDataGraphPipelineSingleNodeCreateInfoARM](VkDataGraphPipelineSingleNodeCreateInfoARM.html)::`pConnections`
**must** be in the layout specified by the `layout` member of the
[VkDataGraphPipelineResourceInfoImageLayoutARM](VkDataGraphPipelineResourceInfoImageLayoutARM.html) that extends the
[VkDataGraphPipelineResourceInfoARM](VkDataGraphPipelineResourceInfoARM.html) structure whose
`descriptorSet` and `binding` members match the `set` and
`binding` members of the
[VkDataGraphPipelineSingleNodeConnectionARM](VkDataGraphPipelineSingleNodeConnectionARM.html) structure for the
connection point
...
[VK_IMAGE_LAYOUT_GENERAL](VkImageLayout.html) layout
