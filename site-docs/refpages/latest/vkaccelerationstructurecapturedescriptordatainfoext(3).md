...
If the `[VK_NV_ray_tracing](VK_NV_ray_tracing.html) extension` is not enabled, then
`accelerationStructureNV` **must** be [VK_NULL_HANDLE](VK_NULL_HANDLE.html)
...
For more information, see the [Vulkan Specification](../../../../spec/latest/chapters/descriptorbuffers.html#VkAccelerationStructureCaptureDescriptorDataInfoEXT).
