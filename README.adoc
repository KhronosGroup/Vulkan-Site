...
[NOTE]
====
This file is largely based on the original upstream README, and *does not
accurately describe* the Vulkan-Site repository.
It is retained in this form to ease synchronization with the upstream.
See link:README.Khronos.adoc for the README specific to this project.
====

The Vulkan-Site project provides the default UI and build system for Khronos
documentation published via {url-antora}[Antora].

The site UI is based on the `master` branch of the {url-project}[Antora
Default UI] project with Khronos additions and modifications, and is kept
loosely in synchronization with the upstream.
`main` was last sychronized with the upstream `master` on 2025-08-11 at
commit 782311344b54aaa734d4c86629b8cd75e4da36d6

See link:LICENSE.adoc for a summary of the licenses of various upstream and
Khronos-authored files in the project.
