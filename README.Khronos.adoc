...
Site] using the link:https://antora.org/[Antora] site generator.
...
* link:https://github.com/KhronosGroup/GLSL[OpenGL Shading Language
  Specification and Extension Specifications]
* More components will be added in the future.

This repository contains only the Antora "`playbook`" describing the various
documentation components and how to integrate them, the site UI, and related
files needed to build the UI and site using the playbook.
...
** `docs-site/package.json` / `package-lock.json` - Node.js infrastructure /
   module database.
* Other files are mostly based on the {url-project}[Antora Default UI]
  project with Khronos additions and modifications, or are site boilerplate
  such as this README.
...
When adding a new repository, or a new component in an existing repository
to the docs site, at least the following files will need to be updated:

* Makefile - when adding a new repository
** Update `REPONAMES` to include the repo name
** Add a new prep-**NAME** target which invokes any needed actions in the new
   repo to prepare its documentation sources for use with Antora
** Add the prep-**NAME** target to `prep-sources`
* CONTRIBUTING.adoc, .github/ISSUE_TEMPLATE/*.md, README.Khronos.adoc - add
  the new repository and/or component to the lists in these files
* docs-site/antora-playbook.yml, docs-site/prepare_playbook.yml - add the
  new repository and/or component to `sources` in the playbook,
  and to `all_sources` in prepare_playbook.yml

Updates of or additions of other files, such as Antora or asciidoctor
extensions and script used during the build, may also be required.
