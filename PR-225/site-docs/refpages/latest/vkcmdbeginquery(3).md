...
If the `queryType` of the pool is [VK_QUERY_TYPE_TIME_ELAPSED_QCOM](VkQueryType.html),
...
If the `queryType` used to create `queryPool` was
[VK_QUERY_TYPE_TIME_ELAPSED_QCOM](VkQueryType.html), the command pool’s queue family
...
[](#VUID-vkCmdBeginQuery-flags-parameter) VUID-vkCmdBeginQuery-flags-parameter
 ...
 If `flags` is not `0`, `flags` **must** be a valid combination of [VkQueryControlFlagBits](VkQueryControlFlagBits.html) values
