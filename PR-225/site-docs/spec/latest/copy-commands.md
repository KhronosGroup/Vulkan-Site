...
- [Image Rotation Addressing](#copies-buffers-images-rotation-addressing)
- [Image_Rotation_Addressing](#copies-buffers-images-rotation-addressing)
...
[](#VUID-VkDeviceMemoryCopyKHR-srcFlags-parameter) VUID-VkDeviceMemoryCopyKHR-srcFlags-parameter
 ...
 If `srcFlags` is not `0`, `srcFlags` **must** be a valid combination of [VkAddressCommandFlagBitsKHR](fundamentals.html#VkAddressCommandFlagBitsKHR) values
...
both a source region and a destination region

* 
[](#VUID-vkCmdCopyBuffer-pRegions-12482) VUID-vkCmdCopyBuffer-pRegions-12482

Each memory location accessed by this command **must** not be accessed by
more than one destination region
...
* 
[](#VUID-VkCopyBufferInfo2-pRegions-12482) VUID-VkCopyBufferInfo2-pRegions-12482
...
The texels in the images are accessed as follows.

Texels at each coordinate (x,y,z,layer) in a image subresource are
accessed, where:

x is in the range [offset.x, offset.x + 
extent.width),

y is in the range [offset.y, offset.y + 
extent.height),

z is in the range [offset.z, offset.z + 
extent.depth),

layer is in the range [subresource.baseArrayLayer,
subresource.baseArrayLayer +  subresource.layerCount)

where for `srcImage`, *offset*, *extent*, and *subresource* are
`srcOffset`, `extent`, and `srcSubresource` for each region,
respectively.

For `dstImage`, *offset*, *extent* , and *subresource* are
`dstOffset`, `extent`, and `dstSubresource` for each region,
respectively, where `dstOffset` and `extent` are first
[size-adjusted](formats.html#formats-size-compatibility) if the format of the
destination image has a different [block extent](formats.html#formats-compatibility-classes) than the format of the source image (e.g. one is a compressed
format).
...
The memory locations accessed by the source and destination images overlap
if they [alias each other](resources.html#resources-memory-aliasing), both
`srcSubresource` and `dstSubresource` overlap the aliased range, and
any of the following are true:

* 
Their contents are not able to be interpreted in consistent ways

* 
If the aliased regions are *host-accessible*, their accessed regions
defined above overlap, where the memory locations accessed by the texels
of a [linear image](../appendices/glossary.html#glossary-linear-resource) is according to its
[VkSubresourceLayout](resources.html#VkSubresourceLayout)

* 
Otherwise, their accessed texel ranges share any of the same texel
coordinates

The memory locations of an image overlap if those two ranges share any of
the same texel coordinates in the same image aspects.
...
* 
[](#VUID-vkCmdCopyImage-commandBuffer-12449) VUID-vkCmdCopyImage-commandBuffer-12449

If the queue family used to create the [VkCommandPool](cmdbuffers.html#VkCommandPool) which
`commandBuffer` was allocated from does not support
[VK_QUEUE_GRAPHICS_BIT](devsandqueues.html#VkQueueFlagBits) and `srcSubresource.aspectMask` or
`dstSubresource.aspectMask` of any element of `pRegions` contain
[VK_IMAGE_ASPECT_DEPTH_BIT](resources.html#VkImageAspectFlagBits) or [VK_IMAGE_ASPECT_STENCIL_BIT](resources.html#VkImageAspectFlagBits),
then both `srcImage` and `dstImage` **must** have a sample count
equal to [VK_SAMPLE_COUNT_1_BIT](limits.html#VkSampleCountFlagBits)
...
`pRegions` and accessed according to [Copy    Image Addressing](#copies-images-addressing)

* 
[](#VUID-vkCmdCopyImage-pRegions-12484) VUID-vkCmdCopyImage-pRegions-12484

Each memory location accessed by this command **must** not be accessed by
more than one destination region, as specified by `pRegions` and
accessed according to [Copy Image Addressing](#copies-images-addressing)
...
In a copy to or from a plane of a [multi-planar    image](formats.html#formats-multiplanar), the [VkFormat](formats.html#VkFormat) of the image and plane **must** be compatible
according to [the description of compatible    planes](formats.html#formats-compatible-planes) for the plane being copied
...
* 
[](#VUID-vkCmdCopyImage2-commandBuffer-12449) VUID-vkCmdCopyImage2-commandBuffer-12449

If the queue family used to create the [VkCommandPool](cmdbuffers.html#VkCommandPool) which
`commandBuffer` was allocated from does not support
[VK_QUEUE_GRAPHICS_BIT](devsandqueues.html#VkQueueFlagBits) and `srcSubresource.aspectMask` or
`dstSubresource.aspectMask` of any element of `pCopyImageInfo->pRegions` contain
[VK_IMAGE_ASPECT_DEPTH_BIT](resources.html#VkImageAspectFlagBits) or [VK_IMAGE_ASPECT_STENCIL_BIT](resources.html#VkImageAspectFlagBits),
then both `srcImage` and `dstImage` **must** have a sample count
equal to [VK_SAMPLE_COUNT_1_BIT](limits.html#VkSampleCountFlagBits)
...
* 
[](#VUID-VkCopyImageInfo2-pRegions-12484) VUID-VkCopyImageInfo2-pRegions-12484
...
enabled, and
the queue family used to create the [VkCommandPool](cmdbuffers.html#VkCommandPool) which
`commandBuffer` was allocated from does not support
...
`pRegions` and accessed according to
[Buffer and Image Addressing](#copies-buffers-images-addressing)

* 
[](#VUID-VkCopyDeviceMemoryImageInfoKHR-pRegions-12473) VUID-VkCopyDeviceMemoryImageInfoKHR-pRegions-12473

Each memory location accessed by this command **must** not be accessed by
more than one destination region, as specified by `pRegions` and
accessed according to [Buffer and    Image Addressing](#copies-buffers-images-addressing)
...
The memory locations accessed by a buffer and image overlap if they
[alias each other](resources.html#resources-memory-aliasing), the `imageSubresource`
overlaps the aliased range, and either:
  * their contents are not interpreted in consistent ways, or
  * their accessed regions defined above overlap, where the memory locations
    accessed by the texels of a [linear image](../appendices/glossary.html#glossary-linear-resource) is
    according to its [VkSubresourceLayout](resources.html#VkSubresourceLayout)
...
* 
[](#VUID-vkCmdCopyBufferToImage-pRegions-12483) VUID-vkCmdCopyBufferToImage-pRegions-12483
...
* 
[](#VUID-vkCmdCopyImageToBuffer-pRegions-12485) VUID-vkCmdCopyImageToBuffer-pRegions-12485
...
* 
[](#VUID-VkCopyBufferToImageInfo2-pRegions-12483) VUID-VkCopyBufferToImageInfo2-pRegions-12483
...
* 
[](#VUID-VkCopyImageToBufferInfo2-pRegions-12485) VUID-VkCopyImageToBufferInfo2-pRegions-12485
...
* 
[](#VUID-VkCopyMemoryToImageInfo-pRegions-09062) VUID-VkCopyMemoryToImageInfo-pRegions-09062

Each memory location accessed by this command **must** not be accessed in
both a source region and a destination region, as specified in
`pRegions` and accessed according to
[Buffer and Image Addressing](#copies-buffers-images-addressing)

* 
[](#VUID-VkCopyMemoryToImageInfo-pRegions-12474) VUID-VkCopyMemoryToImageInfo-pRegions-12474
...
[](#VUID-VkCopyMemoryToImageInfo-flags-parameter) VUID-VkCopyMemoryToImageInfo-flags-parameter
 ...
 If `flags` is not `0`, `flags` **must** be a valid combination of [VkHostImageCopyFlagBits](#VkHostImageCopyFlagBits) values
...
* 
[](#VUID-VkCopyImageToMemoryInfo-pRegions-12475) VUID-VkCopyImageToMemoryInfo-pRegions-12475
...
* 
[](#VUID-VkCopyImageToImageInfo-pRegions-12476) VUID-VkCopyImageToImageInfo-pRegions-12476
...
* 
[](#VUID-VkCopyImageToImageInfo-pRegions-12477) VUID-VkCopyImageToImageInfo-pRegions-12477
...
Each memory location accessed by this command **must** not be accessed by
both a source region and a destination region, and **must** not be accessed
by more than one destination region, as specified in
`copyAddressRange`
...
[](#VUID-VkCopyMemoryIndirectInfoKHR-srcCopyFlags-parameter) VUID-VkCopyMemoryIndirectInfoKHR-srcCopyFlags-parameter
 ...
 If `srcCopyFlags` is not `0`, `srcCopyFlags` **must** be a valid combination of [VkAddressCopyFlagBitsKHR](#VkAddressCopyFlagBitsKHR) values

* 
[](#VUID-VkCopyMemoryIndirectInfoKHR-dstCopyFlags-parameter) VUID-VkCopyMemoryIndirectInfoKHR-dstCopyFlags-parameter

 If `dstCopyFlags` is not `0`, `dstCopyFlags` **must** be a valid combination of [VkAddressCopyFlagBitsKHR](#VkAddressCopyFlagBitsKHR) values
...
Each memory location accessed by this command **must** not be accessed by
both a source region and a destination region, and **must** not be accessed
by more than one destination region, as specified in `copyAddressRange`
and accessed according to [Buffer and    Image Addressing](#copies-buffers-images-addressing)
...
The memory allocation backing `srcAddress` **must** be large enough to
...
The `imageOffset` and `imageExtent` region **must** respect the
image transfer granularity requirements of `commandBuffer`’s command
pool’s queue family, as described in [VkQueueFamilyProperties](devsandqueues.html#VkQueueFamilyProperties)
...
Each memory location accessed by this command **must** not be accessed by
both a source region and a destination region, and **must** not be accessed
by more than one destination region, as specified in `copyBufferAddress`
and accessed according to [Buffer and    Image Addressing](#copies-buffers-images-addressing)
...
The destination offsets form a box which contains all texels which are
written.
A component in `dstOffsets`[1] **can** be smaller than the corresponding
component in `dstOffsets`[0] and forms the same box as swapped
`dstOffsets`.
A destination texel exists if the half-texel coordinates ubase,
vbase and wbase are all contained inside the region.
...
While blitting with equal destination coordinates is not permitted, blitting
with source coordinates being equal is.
However, the resulting coordinate is integer and will approximately land
in-between texels when sampled, which may produce unexpected results.
Especially for nearest filtering, allowed imprecision in generation of the
source coordinate may end up sampling an unexpected texel due to rounding
errors if the coordinate is off by one ULP.
Implementations are not required to implement the formula above exactly as
written.
E.g., varying interpolation in a fragment shader may be used. |
...
If `srcImage` and `dstImage` are the same, and an element of
`pRegions` contains the `srcSubresource` and
`dstSubresource` with matching `mipLevel` and overlapping array
layers, then the `srcImageLayout` and `dstImageLayout` **must** be
[VK_IMAGE_LAYOUT_GENERAL](resources.html#VkImageLayout)
...
unsigned integer color [VkFormat](formats.html#VkFormat), the other **must** also have been
created with an unsigned integer color [VkFormat](formats.html#VkFormat)
...
* 
[](#VUID-vkCmdBlitImage-dstOffsets-12518) VUID-vkCmdBlitImage-dstOffsets-12518

`dstOffsets`[0].x **must** not be equal to `dstOffsets`[1].x

* 
[](#VUID-vkCmdBlitImage-dstOffsets-12519) VUID-vkCmdBlitImage-dstOffsets-12519

`dstOffsets`[0].y **must** not be equal to `dstOffsets`[1].y

* 
[](#VUID-vkCmdBlitImage-dstOffsets-12520) VUID-vkCmdBlitImage-dstOffsets-12520

`dstOffsets`[0].z **must** not be equal to `dstOffsets`[1].z
...
* 
[](#VUID-VkBlitImageInfo2-dstOffsets-12518) VUID-VkBlitImageInfo2-dstOffsets-12518

`dstOffsets`[0].x **must** not be equal to `dstOffsets`[1].x

* 
[](#VUID-VkBlitImageInfo2-dstOffsets-12519) VUID-VkBlitImageInfo2-dstOffsets-12519

`dstOffsets`[0].y **must** not be equal to `dstOffsets`[1].y

* 
[](#VUID-VkBlitImageInfo2-dstOffsets-12520) VUID-VkBlitImageInfo2-dstOffsets-12520
...
[](#VUID-VkResolveImageModeInfoKHR-flags-parameter) VUID-VkResolveImageModeInfoKHR-flags-parameter
 ...
 If `flags` is not `0`, `flags` **must** be a valid combination of [VkResolveImageFlagBitsKHR](#VkResolveImageFlagBitsKHR) values
...
* 
[](#VUID-VkMemoryMarkerInfoAMD-stage-11541) VUID-VkMemoryMarkerInfoAMD-stage-11541
...
* 
[](#VUID-vkCmdWriteBufferMarker2AMD-stage-11541) VUID-vkCmdWriteBufferMarker2AMD-stage-11541
