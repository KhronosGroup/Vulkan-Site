...
[](#VUID-VkSamplerCreateInfo-flags-parameter) VUID-VkSamplerCreateInfo-flags-parameter
 ...
 If `flags` is not `0`, `flags` **must** be a valid combination of [VkSamplerCreateFlagBits](#VkSamplerCreateFlagBits) values
...
In some cases, implementations **may** return **undefined** values instead of the
specified custom border color when used on [VK_QUEUE_COMPUTE_BIT](devsandqueues.html#VkQueueFlagBits) queues
that do not also support [VK_QUEUE_GRAPHICS_BIT](devsandqueues.html#VkQueueFlagBits).
Undefined: values **may** also be returned in task shaders.

|  | Tests added in 2026 revealed issues around custom border colors on AMD in
| --- | --- |
compute-only queues and with task shaders, that were not fixable in
software.
While this will be fixed in future hardware, there is no reasonable
workaround for existing hardware.
Use of standard border colors is unaffected. |
