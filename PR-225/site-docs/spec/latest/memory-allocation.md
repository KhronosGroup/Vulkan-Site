...
| --- | --- |
[VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT](#VkMemoryPropertyFlagBits) on any particular implementation,
there may be different properties which cater to different memory access
patterns.

[VK_MEMORY_PROPERTY_HOST_CACHED_BIT](#VkMemoryPropertyFlagBits) is expected to be used when the
...
[VK_MEMORY_PROPERTY_HOST_COHERENT_BIT](#VkMemoryPropertyFlagBits) without
[VK_MEMORY_PROPERTY_HOST_CACHED_BIT](#VkMemoryPropertyFlagBits) is expected for upload operations
...
support [VK_MEMORY_PROPERTY_HOST_COHERENT_BIT](#VkMemoryPropertyFlagBits) and
[VK_MEMORY_PROPERTY_HOST_CACHED_BIT](#VkMemoryPropertyFlagBits) together.
Either way, applications are expected to look for the first memory type
which supports [VK_MEMORY_PROPERTY_HOST_COHERENT_BIT](#VkMemoryPropertyFlagBits) and use that.
On [VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU](devsandqueues.html#VkPhysicalDeviceType) with a unified memory
architecture, [VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT](#VkMemoryPropertyFlagBits) is less of a concern
since all memory is effectively device-local, but on
[VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU](devsandqueues.html#VkPhysicalDeviceType), there may be a distinction
between a [VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT](#VkMemoryPropertyFlagBits) memory type which is
additionally [VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT](#VkMemoryPropertyFlagBits) or not.

On discrete GPUs, using [VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT](#VkMemoryPropertyFlagBits),
[VK_MEMORY_PROPERTY_HOST_COHERENT_BIT](#VkMemoryPropertyFlagBits), and
[VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT](#VkMemoryPropertyFlagBits) together is intended for small
...
[vkWriteResourceDescriptorsEXT](descriptorheaps.html#vkWriteResourceDescriptorsEXT) and [vkWriteSamplerDescriptorsEXT](descriptorheaps.html#vkWriteSamplerDescriptorsEXT),
updating descriptors in a descriptor buffer with [vkGetDescriptorEXT](descriptorbuffers.html#vkGetDescriptorEXT),
or
writing uniform buffer data.

Some implementations may support host visible types which are
[VK_MEMORY_PROPERTY_HOST_CACHED_BIT](#VkMemoryPropertyFlagBits), where some types support
[VK_MEMORY_PROPERTY_HOST_COHERENT_BIT](#VkMemoryPropertyFlagBits) alongside CACHED, and some types
which do not support [VK_MEMORY_PROPERTY_HOST_COHERENT_BIT](#VkMemoryPropertyFlagBits) in
combination with CACHED.
Unless coherency is required by the application’s usage patterns, the
[VK_MEMORY_PROPERTY_HOST_COHERENT_BIT](#VkMemoryPropertyFlagBits) supporting type is likely to be
less optimal.
...
same memory object.
If [VkBufferDeviceAddressAlignmentAllocateInfoVALVE](#VkBufferDeviceAddressAlignmentAllocateInfoVALVE) is included in the
`pNext` chain of `pAllocateInfo`, the allocation requirements **can**
be further increased to meet alignment requirements which are beyond the
requirements imposed by the implementation.
This increased alignment **can** only be observed through addresses returned by
[vkGetBufferDeviceAddress](resources.html#vkGetBufferDeviceAddress).
Acceleration structure alignment is outside the scope of this alignment.
Such alignment **can** be enforced by [vkCreateAccelerationStructure2KHR](resources.html#vkCreateAccelerationStructure2KHR)
which inherits the device address verbatim.
If the parameters define an [import](#memory-import-operation) operation,
and the same memory object is imported into multiple [VkDeviceMemory](#VkDeviceMemory)
handles,
[VkBufferDeviceAddressAlignmentAllocateInfoVALVE](#VkBufferDeviceAddressAlignmentAllocateInfoVALVE)::`alignment` **may**
be ignored, and the implementation **may** use the alignment of an existing
import of the same handle.
...
* 
[](#VUID-VkMemoryAllocateInfo-None-12523) VUID-VkMemoryAllocateInfo-None-12523

If the parameters define an import operation and the external handle
type is [VK_EXTERNAL_MEMORY_HANDLE_TYPE_DMA_BUF_BIT_EXT](capabilities.html#VkExternalMemoryHandleTypeFlagBitsKHR), the number
of instances allocated **must** be `1`
...
* 
[](#VUID-VkMemoryAllocateInfo-flags-12510) VUID-VkMemoryAllocateInfo-flags-12510

If [VkMemoryAllocateFlagsInfo](#VkMemoryAllocateFlagsInfo)::`flags` does not include
[VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT](#VkMemoryAllocateFlagBitsKHR),
[VkBufferDeviceAddressAlignmentAllocateInfoVALVE](#VkBufferDeviceAddressAlignmentAllocateInfoVALVE)::`alignment`
**must** be 0

* 
[](#VUID-VkMemoryAllocateInfo-alignment-12511) VUID-VkMemoryAllocateInfo-alignment-12511
...
If the `pNext` chain includes a
`VkBufferDeviceAddressAlignmentAllocateInfoVALVE` structure, then that
structure requests a minimum buffer device address alignment for bound
buffers.

The [VkBufferDeviceAddressAlignmentAllocateInfoVALVE](#VkBufferDeviceAddressAlignmentAllocateInfoVALVE) structure is
defined as:
...
[`bufferDeviceAddressAllocationAlignment`](features.html#features-bufferDeviceAddressAllocationAlignment)
**must** be enabled
...
[](#VUID-VkBufferDeviceAddressAlignmentAllocateInfoVALVE-sType-sType) VUID-VkBufferDeviceAddressAlignmentAllocateInfoVALVE-sType-sType
 ...
 `sType` **must** be [VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_ALIGNMENT_ALLOCATE_INFO_VALVE](fundamentals.html#VkStructureType)
...
[](#VUID-VkExportMemoryAllocateInfoNV-handleTypes-parameter) VUID-VkExportMemoryAllocateInfoNV-handleTypes-parameter
 ...
 If `handleTypes` is not `0`, `handleTypes` **must** be a valid combination of [VkExternalMemoryHandleTypeFlagBitsNV](#VkExternalMemoryHandleTypeFlagBitsNV) values
...
[](#VUID-VkImportMemoryWin32HandleInfoNV-handleType-parameter) VUID-VkImportMemoryWin32HandleInfoNV-handleType-parameter
 ...
 If `handleType` is not `0`, `handleType` **must** be a valid combination of [VkExternalMemoryHandleTypeFlagBitsNV](#VkExternalMemoryHandleTypeFlagBitsNV) values
...
* 
[](#VUID-VkMemoryGetFdInfoKHR-handleType-12524) VUID-VkMemoryGetFdInfoKHR-handleType-12524

If `handleType` is
[VK_EXTERNAL_MEMORY_HANDLE_TYPE_DMA_BUF_BIT_EXT](capabilities.html#VkExternalMemoryHandleTypeFlagBitsKHR), `memory` **must**
not have been allocated with multiple instances
...
[](#VUID-VkMemoryAllocateFlagsInfo-flags-parameter) VUID-VkMemoryAllocateFlagsInfo-flags-parameter
 ...
 If `flags` is not `0`, `flags` **must** be a valid combination of [VkMemoryAllocateFlagBits](#VkMemoryAllocateFlagBits) values
...
[](#VUID-vkMapMemory-flags-parameter) VUID-vkMapMemory-flags-parameter
 ...
 If `flags` is not `0`, `flags` **must** be a valid combination of [VkMemoryMapFlagBits](#VkMemoryMapFlagBits) values
...
[](#VUID-VkMemoryUnmapInfo-flags-parameter) VUID-VkMemoryUnmapInfo-flags-parameter
 ...
 If `flags` is not `0`, `flags` **must** be a valid combination of [VkMemoryUnmapFlagBits](#VkMemoryUnmapFlagBits) values
...
or rendered to as a [resolve attachment](features.html#features-externalFormatResolve)
.
